...
EXPRESSION *convertInitToExpression(TYPE *tp, SYMBOL *sp, INITIALIZER *init);
...
int definsert(char *macro, char *end, char *begin, char *text, char *expandedtext, int len, int replen);
void defstringizing(char *macro);
int defreplaceargs(char *macro, int count, char **oldargs, char **newargs, char **expandedargs, char *varargs);

...
# define dxrel 1.490116119384765696e-8
	...
	--n;
	value = __chebyshev_eval(y * 2 - 1, gamcs, ngam) + .9375;
	...
	if (n < 0) {
		...
		mtherr("gammafn", ME_PRECISION);
	    }

	    /* The argument is so close to 0 that the result would overflow. */
	    if (y < xsml) {
		mtherr("gammafn", ME_RANGE);
	...
	if(y <= 50 && y == (int)y) { /* compute (n - 1)! */
	    ...
	    value = exp((y - 0.5) * log(y) - y + M_LN_SQRT_2PI +
			((2*y == (int)2*y)? __stirlerr(y) : __lgammacor(y)));
	}
	if (x > 0)
	    return value;

...
namespace Parser
{
    ...
    void thunkForImportTable(EXPRESSION** exp)
    {
            ...
            ((*exp)->type == en_pc || !(*exp)->v.func->ascall))
        {
            ...
            if (!search)
            {
                ...
                search = (Optimizer::LIST*)(Optimizer::LIST*)Alloc(sizeof(Optimizer::LIST));
    ...
    static EXPRESSION* GetUUIDData(SYMBOL* cls)
    {
        ...
        else
        {
            ...
            data->pointers = (Optimizer::SLCHAR**)Alloc(sizeof(Optimizer::SLCHAR*));
            data->size = 1;
            data->strtype = l_astr;
            data->pointers[0] = (Optimizer::SLCHAR*)Alloc(sizeof(Optimizer::SLCHAR));
    ...
    static EXPRESSION* GetManagedFuncData(TYPE* tp)
    {
        ...
        *save++ = sym->sb->attribs.inheritable.linkage == lk_stdcall;
        ...
        data->pointers[0]->str = (LCHAR*)Alloc((save - buf) * sizeof(Optimizer::SLCHAR));
        ...
        if (sym)
        {
            ...
            *tp = sym->tp;
            ...
            else
            {
                ...
                if (Optimizer::cparams.prm_cplusplus && sym->sb && (sym->sb->storage_class == sc_member || sym->sb->storage_class == sc_mutable))
                {
        ...
        if (hr && hr->p->tp->type == bt_void)
        {
            ...
            while (hr || list)
            {
                ...
                if (!hr || hr->p->tp->type != bt_any)
                {
                                ...
                                else if (ispointer(decl->tp))
                                {
                                    if (!ispointer(list->tp) && (!isarithmeticconst(list->exp) || !isconstzero(decl->tp, list->exp)))
                                    {
                                        if (!isfunction(list->tp) ||
                                            (!isvoidptr(decl->tp) &&
                                             (!isfuncptr(decl->tp) ||
                                              (Optimizer::cparams.prm_cplusplus && !comparetypes(basetype(decl->tp)->btp, list->tp, true)))))
                                            errorarg(ERR_TYPE_MISMATCH_IN_ARGUMENT, argnum, decl, params->sp);
                                    }
                    ...
                    {
                        if ((Optimizer::architecture != ARCHITECTURE_MSIL) || (!isstructured(dest) && (!isarray(dest) || !basetype(dest)->msil)))
    ...
    static bool getSuffixedChar(LEXEME* lex, SYMBOL* funcsp, TYPE** tp, EXPRESSION** exp)
    {
        ...
        enum e_bt tpb = (*tp)->type;
        SYMBOL* sym;
        Optimizer::my_sprintf(name, "%s@%s", overloadNameTab[CI_LIT], lex->suffix);
        sym = LookupSym(name);
        if (sym)
        {
            SYMLIST *hr = sym->tp->syms->table[0], *hr1;
                         ...
                         bool* ismutable, int flags)
    {
        ...
        while (MATCHKW(lex, kw))
        {
            ...
            if (Optimizer::cparams.prm_cplusplus && insertOperatorFunc(kw == lor || kw == land ? ovcl_binary_numericptr : ovcl_binary_int, kw,
                            ...
                            else if (ispointer(basetype(tp1)) || tp1->type == bt_any)
                            {
                                ...
                                if (!comparetypes(*tp, tp1, true))
                                {
                                    ...
                                    if (!found)
                                    {
                                        ...
                                        else if (Optimizer::cparams.prm_cplusplus && !isvoidptr(*tp) && isvoidptr(tp1) && exp1->type != en_nullptr)
                                        {

...
SimpleExpression* GetSymRef(SimpleExpression* n);
	...
	void outputObjFile();
	void mainPreprocess();
	void doPragma(const char* kw, const char* tag);
	void mainPostProcess();
    void compiler_postprocess(const char *);
    void rundown();
	enum e_lk getDefaultLinkage();
	void enter_type(SYMBOL *tp);
	int be_init(COMPILER_PARAMS* cp, ARCH_ASM* asmx, ARCH_DEBUG*dbg);
	void* inlineAsmStmt(void*stmt);
	int parse_codegen(char ch, const char* str);
	int parse_param(char ch, const char* str);
	int insert_noncompile_file(const char*buf);
	void insert_output_file(const char* name);
	bool hasInlineAsm();
	LEXEME* inlineAsm(LEXEME* lex, BLOCKDATA* parent);
	void browsedata(BROWSEINFO* bi);
	void browsefile(BROWSEFILE* bf);
    void outputtypedef(SYMBOL* sym);
	void rundown_dbg();
    void rundown_browsedata();
	void SetDebugger(const char* name);
	void msilCreateProperty(SYMBOL* s1, SYMBOL* s2, SYMBOL* s3);
	bool msilManaged(SYMBOL* s);
    void WinmodeSetup(const char* str);
    void msilWinmodeSetup(const char* str);
	void OutputIcdFile();
	void OutputIntermediate();
	bool msil_managed(SimpleSymbol* sp);
	int parse_codegen(char v, const char *string);
	void AddFunction();
	int GetOutputSize();
...
void PushPopTemplateArgs(SYMBOL* sym, bool push);
TYPE* SolidifyType(TYPE *tp);
TEMPLATEPARAMLIST* SolidifyTemplateParams(TEMPLATEPARAMLIST*);
...
long long imin(long long x, long long y);
...
void gen_vtt(int dataOffset, struct SimpleSymbol* func, struct SimpleSymbol* name);

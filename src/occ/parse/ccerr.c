...
static BOOLEAN ignoreErrtemplateNestingCount(int err)
{
    ...
    case ERR_NEED_TYPEINFO_H:
        return TRUE;
    default:
        return FALSE; // I like consistency
    }
}
BOOLEAN printerrinternal(int err, char *file, int line, va_list args)
{
    ...
    if (IsReturnErr(err) || (errors[err].level & ERROR) || (cparams.prm_ansi && (errors[err].level & ANSIERROR)) || (cparams.prm_cplusplus && (errors[err].level & CPLUSPLUSERROR)))
    {
...
{ // are three levels of VLASHIM pointers REALLY that neccessary?
    while (stmt)
    {
        switch (stmt->type)
        {
        ...
        while (gotop)
        {
            if (gotop->type == v_goto)
            {
                ...
                while (lblp)
                {
                    if (lblp == gotop)
                        after = TRUE;
                    else if (lblp->type == v_label && lblp->label == gotop->label)
                    {
                        int level, minlevel;
                        if (after)
                        {
...
{
    for (int i = 0; i < labelSyms->size; i++)
    {
        for (HASHREC *hr = labelSyms->table[i]; hr; hr = hr->next)
        {
...
void checkUnused(HASHTABLE *syms)
{
    ...
    {
        for (HASHREC *hr = syms->table[i]; hr; hr = hr->next)
        {
            SYMBOL *sp = (SYMBOL *)hr->p;
            if (sp->storage_class == sc_overloads)
                sp = (SYMBOL *)SYMTABBEGIN(sp->tp)->p;
...
{
    for (int i = 0; i < nameSpace->syms->size; i++)
    {
        for (HASHREC *hr = nameSpace->syms->table[i]; hr; hr = hr->next)
        {
            SYMBOL *sp = (SYMBOL *)hr->p;
            if (sp)
            {
...
{
    if ((sp->storage_class == sc_auto || sp->storage_class == sc_register || sp->storage_class == sc_localstatic) && !sp->assigned && !sp->used && !sp->altered)
    {
...
{
    int r = 0;
    for (INITIALIZER *p = spi->init; p; p = p->next)
    {

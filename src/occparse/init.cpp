...
namespace Parser
{
    ...
    static void dumpTLSInitializers(void)
    {
        ...
        if (TLSInitializers)
        {
            ...
            tp->btp->rootType = tp->btp;
            tp->syms = CreateHashTable(1);
            funcsp = makeUniqueID((Optimizer::architecture == ARCHITECTURE_MSIL) ? sc_global : sc_static, tp, nullptr, "__TLS_DYNAMIC_STARTUP__");
            funcsp->sb->inlineFunc.stmt = stmtNode(nullptr, nullptr, st_block);
            funcsp->sb->inlineFunc.stmt->lower = st;
            tp->sp = funcsp;
            SetLinkerNames(funcsp, lk_none);
    ...
    static void dumpTLSDestructors(void)
    {
        ...
        if (TLSDestructors)
        {
            ...
            tp->btp->rootType = tp->btp;
            tp->syms = CreateHashTable(1);
            funcsp = makeUniqueID((Optimizer::architecture == ARCHITECTURE_MSIL) ? sc_global : sc_static, tp, nullptr, "__TLS_DYNAMIC_RUNDOWN__");
            funcsp->sb->inlineFunc.stmt = stmtNode(nullptr, nullptr, st_block);
            funcsp->sb->inlineFunc.stmt->lower = st;
            tp->sp = funcsp;
            SetLinkerNames(funcsp, lk_none);
        ...
        else
        {
            ...
            else
            {
                ...
                if (itype->type != bt_templateparam && !templateNestingCount)
                {
                    ...
                    if (Optimizer::cparams.prm_cplusplus && (isarithmetic(itype) || basetype(itype)->type == bt_enum) && isstructured(tp))
                    {
                        castToArithmetic(false, &tp, &exp, (enum e_kw) - 1, itype, true);
                    }
                    if (isstructured(tp))
        ...
        else
        {
            ...
            while (lex)
            {
                ...
                if ((((sc != sc_auto && sc != sc_register) || needend) && MATCHKW(lex, comma)) || MATCHKW(lex, end))
                {
                    ...
                    {
                        if (desc->hr && Optimizer::cparams.prm_cplusplus && isstructured(itype) && !basetype(itype)->sp->sb->trivialCons)
                        {
                            while (desc->hr)
                            {
    ...
    LEXEME* initialize(LEXEME* lex, SYMBOL* funcsp, SYMBOL* sym, enum e_sc storage_class_in, bool asExpression, int flags)
    {
        ...
        else if ((Optimizer::cparams.prm_cplusplus || ((Optimizer::architecture == ARCHITECTURE_MSIL) && isstructured(sym->tp) && !basetype(sym->tp)->sp->sb->trivialCons)) &&
                 sym->sb->storage_class != sc_typedef && sym->sb->storage_class != sc_external && !asExpression)
        {
            TYPE* t =
                ((Optimizer::architecture == ARCHITECTURE_MSIL) && Optimizer::cparams.msilAllowExtensions) ? find_boxed_type(sym->tp) : 0;
            if (!t || !search(overloadNameTab[CI_CONSTRUCTOR], basetype(t)->syms))
                t = sym->tp;
            if (isstructured(t))
            {
            ...
            if (!templateNestingCount)
            {
                if (!sym->sb->init)
                {
                    if (!asExpression)
                    {
                        ...
                        {
                            if (!sym->sb->assigned &&
                                (!isstructured(tp) || !Optimizer::cparams.prm_cplusplus || (basetype(tp)->sp->sb->trivialCons && hasData(tp))))
                            {

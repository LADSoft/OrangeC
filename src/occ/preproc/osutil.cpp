...
extern HASHTABLE *globalTags, *globalSyms;
extern char infile[];
...
#ifndef CPREPROCESSOR
    if (cparams.prm_compileonly || !chosenAssembler->insert_noncompile_file || !chosenAssembler->insert_noncompile_file(buffer))
#endif
    {
        AddExt(buffer, ".C");
...
void dumperrs(FILE* file)
{
    ...
    if (cparams.prm_listfile)
    {
        ...
        list_table(globalSyms, 0);
        fprintf(listFile, "******** Global Tags ********\n");
        list_table(globalTags, 0);
    }

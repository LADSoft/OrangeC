                ...
                join:
                    ...
                    if (name)
                    {
                        ...
                        if (!skip)
                        {
                            ...
                            if (exp && exp->type == en_templateparam)
                            {
...
SYMBOL* TemplateClassInstantiateInternal(SYMBOL* sym, TEMPLATEPARAMLIST* args, bool isExtern)
{
    ...
    if (!isExtern)
    {
        ...
        {
            if (sym->name[2] == 'm' && !strcmp(sym->name, "__make_integer_seq"))
                return MyMakeIntegerSeq(sym->templateParams->next);
            else if (sym->name[2] == 't' && !strcmp(sym->name, "__type_pack_element"))
                return MyTypePackElement(sym->templateParams->next);
        }

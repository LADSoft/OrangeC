...
LEXEME* expression_func_type_cast(LEXEME* lex, SYMBOL* funcsp, TYPE** tp, EXPRESSION** exp, int flags)
{
    ...
    else if (!cparams.prm_cplusplus && ((architecture != ARCHITECTURE_MSIL) || cparams.msilAllowExtensions))
    {
        *exp = intNode(en_c_i, 0);
        errortype(ERR_IMPROPER_USE_OF_TYPE, *tp, nullptr);
        errskim(&lex, skim_semi);
    ...
    else
    {
        ...
        if ((architecture == ARCHITECTURE_MSIL) && isarithmetic(*tp))
        {
...
bool insertOperatorParams(SYMBOL* funcsp, TYPE** tp, EXPRESSION** exp, FUNCTIONCALL* funcparams, int flags)
{
    ...
    if (s2)
    {
        ...
        while (hrs)
        {
            SYMLIST* ins = (SYMLIST*)Alloc(sizeof(SYMLIST));
            ins->p = hrs->p;
            *hrd = ins;
            hrd = &(*hrd)->next;
            hrs = hrs->next;

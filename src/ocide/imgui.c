...
static char *fontHints[] =
{
    "Font" , "Point Size", "Bold" , "Italic", "Underline"
};
static TBBUTTON fontButtons[] = 
{
    {
        170, 10000, TBSTATE_WRAP, TBSTYLE_SEP | TBSTYLE_FLAT, {0}, 0, -1
    }
    , 
    {
    ...
    switch(iMessage)
    {
        case WM_NOTIFY:
         ...
         case WM_CREATE:
            ...
            p->hwndScreenColorToolbar = CreateWindowEx(0, TOOLBARCLASSNAME, (LPSTR)NULL,
                WS_CHILD | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | TBSTYLE_TRANSPARENT | CCS_NODIVIDER, 
    ...
    switch(iMessage)
    {
        case WM_NOTIFY:
         ...
         case WM_CREATE:
            p = (IMGDATA *) (((LPCREATESTRUCT) lParam)->lpCreateParams); 
            SetWindowLong(hwnd, GWL_USERDATA, (LONG)p);
            p->hwndColorToolbar = CreateWindowEx(0, TOOLBARCLASSNAME, (LPSTR)NULL,
...
LRESULT CALLBACK  ColorWndProc(HWND hwnd, UINT iMessage, WPARAM wParam,
    LPARAM lParam)
{
    PAINTSTRUCT paint;
    HDC dc;
    ...
    switch(iMessage)
    {
        case WM_NOTIFY:
         ...
         case WM_CREATE:
            p = (IMGDATA *) (((LPCREATESTRUCT) lParam)->lpCreateParams); 
            SetWindowLong(hwnd, GWL_USERDATA, (LONG)p);
            p->hwndColor2 = CreateWindowEx(0, szColor2ClassName, "",
                WS_CHILD | WS_VISIBLE | WS_CLIPCHILDREN | WS_CLIPSIBLINGS,
                0, 0, CW_USEDEFAULT, CW_USEDEFAULT,
                hwnd, 0, hInstance, p);
            p->hwndScreenColor = CreateWindowEx(0, szScreenColorClassName, "",
                WS_CHILD | WS_VISIBLE | WS_CLIPCHILDREN | WS_CLIPSIBLINGS,
            ...
            p->hwndSelectToolbar = CreateToolbarEx(hwnd, WS_CHILD | WS_VISIBLE | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | TBSTYLE_TRANSPARENT | CCS_NODIVIDER, 
                ID_COLORTOOLBAR, 1, hInstance, 0/*IDB_TOOLBAR*/, 
                buttons, 1, 24, 24, 24, 24, sizeof(TBBUTTON));
            ...
            SendMessage(p->hwndSelectToolbar, TB_BUTTONSTRUCTSIZE, sizeof(TBBUTTON),
                0);
        ...
        case WM_SIZE:
            ...
            */
            ...
            rf.bottom = HIWORD(lParam) ;
            MoveWindow(p->hwndScreenColor, rf.left, rf.top, 
                       p->screenColorTbSize.right - p->screenColorTbSize.left, 
                       rf.bottom - rf.top, 1);
            ...
            rf.bottom = HIWORD(lParam) ;
            MoveWindow(p->hwndColor2, rf.left, rf.top, 
                       p->colorTbSize.right - p->colorTbSize.left+4,
...
static void unselectDo(IMGDATA *p)
{
    if (p->selected)
    {
        BitBlt(p->res->hdcImage, p->selectRect.left + p->selectMovePos.x - p->selectStartPos.x,
               p->selectRect.top + p->selectMovePos.y - p->selectStartPos.y,
               p->selectRect.right - p->selectRect.left, p->selectRect.bottom - p->selectRect.top,
               p->selectDC, 0, 0, SRCCOPY);
...
static void selectDo(IMGDATA *p)
{
    if (!p->selected)
    {
        ...
        p->selectbmp = CreateCompatibleBitmap(p->res->hdcImage,
                        ...
                        p->selectRect.bottom - p->selectRect.top);
        if (!p->selectbmp)
        {
            DeleteDC(p->selectDC);
            return ;
    ...
    switch(iMessage)
    {
        ...
        case WM_CREATE:
            ...
            SendMessage(p->hwndControlToolbar, TB_BUTTONSTRUCTSIZE, sizeof(TBBUTTON),
                0);
            SendMessage(p->hwndControlToolbar, TB_SETROWS, MAKEWPARAM(5, FALSE), 
                (LPARAM) &p->controlTbSize);
            return 0;
        case WM_DESTROY:
            p = (IMGDATA *)GetWindowLong(hwnd, GWL_USERDATA);
        ...
        case WM_SIZE:
            ...
            rf.right = LOWORD(lParam);
            rf.bottom = HIWORD(lParam);
            MoveWindow(p->hwndControlToolbar, rf.left, rf.top,
                       p->controlTbSize.right - p->controlTbSize.left,
    ...
    switch(iMessage)
    {
        case WM_NOTIFY:
            ...
            if (((LPNMHDR)lParam)->code == NM_CUSTOMDRAW)
            {
                ...
                {
                    if (lptcd->nmcd.dwItemSpec >= IDM_BRUSH_ROUND_7 
                        && lptcd->nmcd.dwItemSpec <= IDM_BRUSH_UP_2)
                    {
                        ...
                        switch(lptcd->nmcd.dwItemSpec)
                        {
                            case IDM_BRUSH_ROUND_7:
                                Ellipse(lptcd->nmcd.hdc, lptcd->nmcd.rc.left,
                                          lptcd->nmcd.rc.top + 2,
                                          lptcd->nmcd.rc.left + 7,
                                          lptcd->nmcd.rc.top + 9);
                                break;
                            case IDM_BRUSH_ROUND_4:
                                Ellipse(lptcd->nmcd.hdc, lptcd->nmcd.rc.left + 2,
                                          lptcd->nmcd.rc.top + 3,
                                          lptcd->nmcd.rc.left + 6,
                                          lptcd->nmcd.rc.top + 7);
                                break;
                            case IDM_BRUSH_ROUND_2:
                                Ellipse(lptcd->nmcd.hdc, lptcd->nmcd.rc.left+3,
                                          lptcd->nmcd.rc.top+4,
                                          lptcd->nmcd.rc.left + 5,
                                          lptcd->nmcd.rc.top + 6);
                                break;

                            case IDM_BRUSH_SQUARE_7:
                                Rectangle(lptcd->nmcd.hdc, lptcd->nmcd.rc.left,
                                          lptcd->nmcd.rc.top + 2,
                                          lptcd->nmcd.rc.left + 7,
                                          lptcd->nmcd.rc.top + 9);
                                break;
                            case IDM_BRUSH_SQUARE_4:
                                Rectangle(lptcd->nmcd.hdc, lptcd->nmcd.rc.left + 2,
                                          lptcd->nmcd.rc.top + 3,
                                          lptcd->nmcd.rc.left + 6,
                                          lptcd->nmcd.rc.top + 7);
                                break;
                            case IDM_BRUSH_SQUARE_2:
                                Rectangle(lptcd->nmcd.hdc, lptcd->nmcd.rc.left+3,
                                          lptcd->nmcd.rc.top+4,
                                          lptcd->nmcd.rc.left + 5,
                                          lptcd->nmcd.rc.top + 6);
                                break;
            ...
            switch(LOWORD(wParam))
            {
                case IDM_AUXTOOLBAR:
                    ...
                    if (p->hwndSelectedToolbar)
                    {
                        ...
                        SendMessage(p->hwndSelectedToolbar, TB_SETROWS,
                                    MAKEWPARAM(rows, FALSE), (LPARAM)&rc);								  
            ...
            if (p->drawMode == IDM_TEXT && p->selected)
            {
                ...
                if (p->textLen)
                {
                    ...
                    if (n != CB_ERR && m != CB_ERR)
                    {
                        ...
                        BitBlt(hdcMem, 0, 0, p->res->width, p->res->height,
                               p->res->hdcImage, 0, 0, SRCCOPY);
                        ...
                        BitBlt(p->res->hdcImage, 0, 0, p->res->width, p->res->height,
                               hdcMem, 0, 0, SRCCOPY);
                        bmp = SelectObject(hdcMem, bmp);
                        DeleteObject(bmp);
                        hf = SelectObject(hdcMem, hf);
        ...
        case WM_COMMAND:
            switch(LOWORD(wParam))
            {
                ...
                case IDM_CLEARSELECTION:
                {
                    ...
                    if (selected)
                        PatBlt(p->res->hdcImage, p->selectRect.left, p->selectRect.top, 
                               p->selectRect.right - p->selectRect.left, 
                ...
                case IDM_ROT90CW:
                {
                    ...
                    FlipOrRotate(p, &xx, 
                                 ...
                                 p->selectRect.right- p->selectRect.left,
                                 p->selectRect.bottom - p->selectRect.top,
                                 -p->selectRect.bottom+xx.eDx, p->selectRect.left + xx.eDy,
                                 p->selectRect.bottom - p->selectRect.top,
                                 p->selectRect.right- p->selectRect.left
                                 );
                ...
                case IDM_ROT90CCW:
                {
                    ...
                    FlipOrRotate(p, &xx, 
                                 ...
                                 p->selectRect.right- p->selectRect.left,
                                 p->selectRect.bottom - p->selectRect.top,
                                 p->selectRect.top+xx.eDx, -p->selectRect.right + xx.eDy,
                                 p->selectRect.bottom - p->selectRect.top,
                                 p->selectRect.right- p->selectRect.left
                                 );
                ...
                {			
                    ...
                    if (p->fillType != FT_HOLLOW)
                    {
                        ...
                        else
                        {
                            ...
                            BitBlt(p->res->hdcImage, 0, 0, p->res->width, 
                                   p->res->height, memDC, 0, 0, DSPDxax); 
                        }
                    }
                    else
                    {
...
//			SendMessage(p->hwndStatus, SB_SETTEXT, 1, (LPARAM)buf);
        ...
        case WM_PAINT:
            ...
            if (p->res)
            {
                ...
                GetClientRect(hwnd, &rc);
                StretchBlt(hdcMem, 0, 0, p->res->width * p->zoom, p->res->height*p->zoom, 
                           p->res->hdcImage, 0, 0, p->res->width, p->res->height, SRCCOPY);
                if (p->rubber)
                {
                    HBRUSH brush;
                    ...
                    StretchBlt(hdcMem, 0, 0, p->res->width * p->zoom, p->res->height*p->zoom, 
                               p->rubber->hdcRubberBorder, 0, 0, p->res->width, p->res->height, DSPDxax);
                    brush = SelectObject(hdcMem, brush);
                    DeleteObject(brush);
                    if (p->drawMode == IDM_ELLIPSE || p->drawMode == IDM_RECTANGLE || p->drawMode == IDM_ROUNDEDRECTANGLE)
                    ...
                    brush = SelectObject(hdcMem, brush);
                    StretchBlt(hdcMem, 0, 0, p->res->width * p->zoom, p->res->height*p->zoom, 
                               p->rubber->hdcRubberFill, 0, 0, p->res->width, p->res->height, DSPDxax);
                    brush = SelectObject(hdcMem, brush);
                    DeleteObject(brush);
                }
                if (p->drawMode == IDM_ERASE && p->mouseInClient)
                {
                    Rectangle(hdcMem, (p->cursorPos.x - p->eraseWidth / 2) * p->zoom, 
                          (p->cursorPos.y - p->eraseWidth / 2) * p->zoom,
                          (p->cursorPos.x + p->eraseWidth/ 2 + 1) * p->zoom, 
                ...
                if (p->selected)
                {
                    ...
                    StretchBlt(hdcMem, (p->selectRect.left + p->selectMovePos.x - p->selectStartPos.x) *p->zoom,
                       ...
                       (p->selectRect.bottom - p->selectRect.top) * p->zoom,
                           p->selectDC, 0, 0, 
                        p->selectRect.right - p->selectRect.left, 
                        p->selectRect.bottom - p->selectRect.top,
                        SRCCOPY);
                    pen = SelectObject(hdcMem, pen);
                    MoveToEx(hdcMem, (p->selectRect.left+ p->selectMovePos.x - p->selectStartPos.x) *p->zoom, (p->selectRect.top+ p->selectMovePos.y - p->selectStartPos.y) * p->zoom, NULL);
                    LineTo(hdcMem, (p->selectRect.left+ p->selectMovePos.x - p->selectStartPos.x) *p->zoom, (p->selectRect.bottom+ p->selectMovePos.y - p->selectStartPos.y) * p->zoom);
                    LineTo(hdcMem, (p->selectRect.right+ p->selectMovePos.x - p->selectStartPos.x) *p->zoom, (p->selectRect.bottom+ p->selectMovePos.y - p->selectStartPos.y) * p->zoom);
                    LineTo(hdcMem, (p->selectRect.right+ p->selectMovePos.x - p->selectStartPos.x) *p->zoom, (p->selectRect.top+ p->selectMovePos.y - p->selectStartPos.y) * p->zoom);
                    LineTo(hdcMem, (p->selectRect.left+ p->selectMovePos.x - p->selectStartPos.x) *p->zoom, (p->selectRect.top+ p->selectMovePos.y - p->selectStartPos.y) * p->zoom);
                    pen = SelectObject(hdcMem, pen);
                    DeleteObject(pen);
                ...
                BitBlt(dc, 0, 0, rc.right, rc.bottom,
                       hdcMem, p->showX, p->showY, SRCCOPY);
    ...
    LPARAM lParam)
{
    ...
    switch(iMessage)
    {
        ...
        case WM_CREATE:
            ...
            SetWindowLong(hwnd, GWL_USERDATA, (LONG)p);
            p->hwndDrawArea = CreateWindowEx(0, szDrawAreaClassName, "",
        ...
        case WM_SIZE:
        {
            ...
            MoveWindow(p->hwndDrawArea, 0, 0, width + 2 * GetSystemMetrics(SM_CXBORDER), height + 2 * GetSystemMetrics(SM_CYBORDER), 1);
        }
            break;
        case WM_LBUTTONUP:
            p = (IMGDATA *)GetWindowLong(hwnd, GWL_USERDATA);
        ...
        case WM_COMMAND:
            switch(LOWORD(wParam))
            {
                case IDM_SHOWFONTTB:
                    ...
                    p = (IMGDATA *)GetWindowLong(hwnd, GWL_USERDATA);
                    p->showFontTB = lParam;
                    SetWindowPos(p->hwndFontTB, 0, 0, 0, 0, 0, SWP_NOZORDER | SWP_NOSIZE | SWP_NOACTIVATE | SWP_NOMOVE | (lParam ? SWP_SHOWWINDOW : SWP_HIDEWINDOW));
...
//                &fontButtons, 5, 16, 16, 16, 16, sizeof(TBBUTTON));
            p->hwndFontCombo = CreateWindowEx(0, "COMBOBOX", "", WS_CHILD + WS_BORDER +
                WS_VISIBLE + WS_TABSTOP + WS_VSCROLL + CBS_DROPDOWN + CBS_AUTOHSCROLL + CBS_SORT | CBS_OWNERDRAWFIXED | CBS_HASSTRINGS, 
            ...
            p->hwndPointSizeCombo = CreateWindowEx(0, "COMBOBOX", "", WS_CHILD + WS_BORDER +
            ...
            p->hwndControl = CreateWindowEx(0, szControlClassName, "",
                WS_CHILD | WS_VISIBLE | WS_CLIPCHILDREN | WS_CLIPSIBLINGS,
                CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT,
                hwnd, 0, hInstance, p);
            p->hwndAux = CreateWindowEx(WS_EX_CLIENTEDGE, szAuxClassName, "",
                WS_CHILD | WS_VISIBLE | WS_CLIPCHILDREN | WS_CLIPSIBLINGS,
                CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT,
                hwnd, 0, hInstance, p);
            p->hwndColor = CreateWindowEx(0, szColorClassName, "",
                WS_CHILD | WS_VISIBLE |  WS_CLIPCHILDREN | WS_CLIPSIBLINGS,
                CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT,
                hwnd, 0, hInstance, p);
...
//                "    ");
        ...
        case WM_SIZE:
            ...
            MoveWindow(p->hwndStatus, rf.left, rf.top, rf.right - rf.left,
                rf.bottom - rf.top, 1);
...
//                "    ");
            ...
            MoveWindow(p->hwndColor, rf.left, rf.top, rf.right - rf.left, 
                       rf.bottom - rf.top, 1);                       
            ...
            if (p->res->type != FT_BMP)
            {
                ...
                rf.left = p->controlTbSize.right + 3 * GetSystemMetrics(SM_CXDLGFRAME);
                rf.right = LOWORD(lParam);
                MoveWindow(p->hwndImageTab, rf.left, rf.top, rf.right - rf.left,
                       rf.bottom - rf.top, 1);
            }
            rf.bottom = rf.top;
            if (p->showFontTB)
            ...
            rf.left = p->controlTbSize.right + 3 * GetSystemMetrics(SM_CXDLGFRAME);
            rf.right = LOWORD(lParam);
            MoveWindow(p->hwndWorkspace, rf.left, rf.top, rf.right - rf.left,
                       rf.bottom - rf.top, 1);
            ...
            if (p->showFontTB)
            {
                ...
                rf.left = p->controlTbSize.right + 3 * GetSystemMetrics(SM_CXDLGFRAME);
                rf.right = LOWORD(lParam);
                MoveWindow(p->hwndFontTB, rf.left, rf.top, rf.right - rf.left,
                           rf.bottom - rf.top, 1);
            }
            ...
            MoveWindow(p->hwndControl, rf.left, rf.top, rf.right - rf.left, 
                       rf.bottom - rf.top, 1);
            ...
            MoveWindow(p->hwndAux, rf.left + GetSystemMetrics(SM_CYDLGFRAME), rf.top,
                       rf.right - rf.left - 2 * GetSystemMetrics(SM_CYDLGFRAME), 
                       rf.bottom - rf.top, 1);

...
This is a version of the Orange C compiler that does MSIL code generation for the .NET Framework and .NET core.
...
`mscorlib` is also automatically loaded for framework projects and its static functions are available for use.   

If compiling for .Net Core 'System.Runtime' is used instead of 'mscorlib'.
...
The `/Wd` switch means make a DLL.  `/Wg` means Windows GUI.   `/Wc` (the default) means Windows console.   Adding `l` on the end of a `/W` switch (e.g. `/Wcl`) means load `lscrtl.dll` as the unmanaged runtime, instead of `msvcrt.dll`.   You might want to do this to get access to C99 and C11 functions.
...
the switch '--netcore:x'  tells the compiler to compile against the given version of .net core.   use --netcore:0 to compile against the latest version.

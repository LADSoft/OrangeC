...
char* segnames[] = {0,         "code",     "data",     "bss",        "string",     "const",
...
static void link_trailer(void) { emit_record_ieee("ME.\r\n"); }
...
void link_Segs(void)
{
    ...
    for (i = 1; i < MAX_SEGS; i++)
    {
        if (segxlattab[i])
        {
            emit_record_ieee("ST%X,%s,%03X%s.\r\n", segxlattab[i], segchars[i], strlen(segnames[i]), segnames[i]);
    ...
    while (v)
    {
        ...
        beDecorateSymName(buf + 3 + (v->sp->decoratedName[0] != '@'), v->sp);
        emit_record_ieee("ST%X,%s,M,V,%03X%s.\r\n", i, v->data ? segchars[dataseg] : segchars[codeseg], strlen(buf), buf);
        emit_record_ieee("SA%X,%x.\r\n", i, 4);
        emit_record_ieee("ASS%X,%lX.\r\n", i++, v->seg->curlast);
        v = v->next;
    }
...
void link_putpub(SYMBOL* sp, char sel)
{
    ...
    if (isconst(sp->tp) && isint(sp->tp) && sp->storage_class == sc_static)
        return;
...
void link_Publics(void)
{
    ...
    {
        SYMBOL* sp = lf->data;
        if ((sp->storage_class == sc_global || sp->storage_class == sc_localstatic ||
             (sp->storage_class == sc_constant && !sp->parent) || (cparams.prm_debug && sp->parentClass && !isfunction(sp->tp)) ||
...
void link_PassSeperator(void) { emit_record_ieee("CO101,006ENDSYM.\r\n"); }
    ...
    switch (fixup->fmode)
    {
        ...
        case fm_symbol:
        {
            ...
            if (sp->wasUsing && sp->mainsym)
                sp = sp->mainsym;
            if (!sp->dontinstantiate &&
                ((isfunction(sp->tp) && sp->inlineFunc.stmt) || (!isfunction(sp->tp) && sp->linkage == lk_virtual) ||
                 sp->storage_class == sc_global || sp->storage_class == sc_static || sp->storage_class == sc_localstatic ||
        ...
        case fm_label:
            ...
            if (rel)
            {
                ...
                */
                {
                    sprintf(buf, "R%X,%X,+,4,-,P,-", xseg, LabelAddress(fixup->label) + offs);
                }
            }
            else
            {
                sprintf(buf, "R%X,%X,+", xseg, LabelAddress(fixup->label) + offs);
            }
            /* segment relative */
            break;
    }
                ...
                case e_ad_funcdata:
                    if (sp->linkage == lk_virtual)
                        sprintf(buf1, "R%X", (int)sp->value.i);
                              ...
                              ((sp->storage_class == sc_member || sp->storage_class == sc_virtual) && isfunction(sp->tp) &&
                               sp->inlineFunc.stmt)))
...
void link_BrowseInfo(void)
{
    if (browseInfo && cparams.prm_browse)
    {
        ...
        {
            char buf1[4096];
            sprintf(buf1, "%x,%x,%d,%d,%d,%03X%s", browseInfo->type, browseInfo->flags, browseInfo->filenum, browseInfo->lineno,

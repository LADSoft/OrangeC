...
static void LoadFile(WINDOW wnd)
{
    ...
    if ((fp = fopen(wnd->extension, "rt")) != NULL)    {
		while (!feof(fp))	{
			handshake();
			Buf = DFrealloc(Buf, recptr+256);
			memset(Buf+recptr, 0, 256);
        	fgets(Buf+recptr, 256, fp);
...
static void SaveFile(WINDOW wnd, int Saveas)
{
    ...
    if (wnd->extension == NULL || Saveas)    {
        ...
        if (SaveAsDialogBox("*.*", NULL, FileName))    {
			...
			{
				SendMessage(GetParent(wnd), TAB_REMOVE, (PARAM)wnd->extension, NULL);
                free(wnd->extension);
			}
			...
			SendMessage(GetParent(wnd), TAB_ADD, (PARAM)FileName, (PARAM)wnd);
        }
        else
            return;
    }
...
static int EditorProc(WINDOW wnd,MESSAGE msg,PARAM p1,PARAM p2)
{
    ...
    switch (msg)    {
        ...
        case CLOSE_WINDOW:
            ...
            wndpos = 0;
            if (wnd->extension != NULL)    {
				SendMessage(GetParent(wnd), TAB_REMOVE, (PARAM)wnd->extension, (PARAM)wnd);
                free(wnd->extension);
                wnd->extension = NULL;
            }
            break;
...
void reconfigTabBar(WINDOW wnd)
{
    ...
    while (wnd1 != NULL)    {
		...
		{
			SendMessage(wnd, TAB_ADD, (PARAM)wnd1->extension, (PARAM)wnd1);
		}
       	wnd1 = NextWindow(wnd1);
	}

...
enum e_lk get_dll_linkage(SimpleSymbol*sym);                /* get dll linkage corresponding to command line switches */

                              /* Dbgfmt.c */

void dbginit(void);
void debug_outline(char *file, int lineno);           /* output a line number */
void debug_outputtypedef(SimpleSymbol*sym) ; /* dump the typedef HASHTABLE */
...
void omfputext(SimpleSymbol*sym);
void omfExtDefs(void);
void omfputimport(SimpleSymbol*sym);
void omfImports(void);
int omfgetseg(SimpleSymbol*sym);
void omfputpub(SimpleSymbol*sym);
void omfpubdumphash(HASHTABLE *syms);
void omfPublics(void);
void omfputexport(SimpleSymbol*sym);
...
void oa_gen_vtt(VTABENTRY *entry, SimpleSymbol*func);
...
void gen_symbol_fixup(enum mode xmode, int seg, int address, SimpleSymbol*pub);

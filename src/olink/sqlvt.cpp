...
int SQL3VirtualTable::thunkCreate(  sqlite3 *dbx,  void *pAux,  int argc, 
...
int SQL3VirtualTable::thunkFilter(  sqlite3_vtab_cursor *cur, 
...
int SQL3VirtualTable::iBestIndex(sqlite3_index_info *pIdxInfo)
{
  return SQLITE_OK;
}
...
int SQL3VirtualTable::iNext(void *userCursor)
{
    return Next(userCursor);
}
int SQL3VirtualTable::iEof(void *userCursor)
{
    return Eof(userCursor);
}
int SQL3VirtualTable::iColumn(void *userCursor, sqlite3_context *ctx, int i)
{
    return Column(userCursor, ctx, i);
}
int SQL3VirtualTable::iRowid(void *userCursor, sqlite_int64 *pRowid)
{
    return RowId(userCursor, pRowid);
}
...
int IntegerColumnsVirtualTable::Filter(void *cursor, int idxNum, const char *idxStr,
            int argc, sqlite3_value **argv)
{
    struct lfcurs * q = (struct lfcurs *)cursor;
    q->i = 0;
...
char *IntegerColumnsVirtualTable::GetVTabDeclarator()
{
    return (char *)declarator.c_str();
}
char *IntegerColumnsVirtualTable::GetName()
{

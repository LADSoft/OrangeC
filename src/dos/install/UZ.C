...
{
    byte_count += 1024;
    fread(inflatebuf,1,1024,FFzipfile) ;
    inflatepos = 0 ;
}
...
void ExpandTables(UINT16 *dest2, UINT16 *dest, UINT8 *source, SINT32 len)
{
    ...
    memset(intermed_tab_1,0,sizeof(intermed_tab_1)*2) ;
...
UINT32 DoExistsPrompt(char *name)
{
    ...
    if (fil)
    {
        ...
        fclose(fil);
        // exists...
        PrintMessage("%s exists.  Overwrite (Y/N/A/Q)? ", name); 
        while (!done)
        {
            int n;
            done = 1;
...
UINT32 unzipOneFile(char *path, ZIPDIR *direntry, UINT32 sfxOffset) {
    ...
    PrintMessage ("File: #%s %s# ",path,pathandname) ;
    ...
    if (fileentry.bitFlags & 1)
    {
        ...
        {
            PrintMessage("bad password (SKIPPED)\r\n");
            return 0;
        }
        ...
        if (!FFzipfile)
        {
            ...
            PrintMessage ("Can't open temp file :-(") ;
            return 1;
        }
    }
    if (qq5 == 8) /* then */ {
      PrintMessage ("Deflated ") ; /* 8 = Deflate-32 */
    ...
    if ((qq5 != 0) && (qq5 != 8)) /* then */ {
      rv=2;
      PrintMessage ("Unsupported compression method (%d) :-(",qq5);	 
    } ; /* endif */
    ...
    PrintMessage("%x:%x", ~currentcrc, direntry->crc32);
    if (rv==0) /* then */ {
      if (~currentcrc != direntry->crc32) /* then */ {
        PrintMessage ("CRC FAILED :-(") ; rv = 1 ;
      } else {
        PrintMessage ("OK");
      } ; /* endif */
    } /* endif */
...
UINT32 unzip(char *filename, char *path) {
    ...
    if (!FFzipfile) /* then */ {
        PrintMessage("Cannot open input file #%s# :-(", filename) ;
        vntype=99 ;
    } else {	
        qq2 = fread(buf7,1,32,FFzipfile) ;
        if (qq2!=32) /* then */ {
          PrintMessage ("File is faulty/empty/ridiculously small :-(");
          vntype=99 ;
        } else {	
		  if (buf7[0] == 'M' && buf7[1] == 'Z')
		  {
			...
			{
				PrintMessage("Invalid file format");
			    vntype = 99;
			}
			else
			{
		  ...
		  if (vntype != 99)
		  {
              ...
              if (vntype==0) /* then */ {
                ...
                PrintMessage ("ZIP file format detected");
                PrintMessage ("\r\n\r\n") ;
               } ; /* endif */
          } ; /* endif */
        } ; /* endif */
...
UINT32 main (UINT32 argc, char *argv[]) {
    ...
    if (argc == 1 || argc > 3) /* then */ {
      ...
      PrintMessage ("Usage: UZ [/Q] [/ppassword] file [path]") ; qq1=1 ;

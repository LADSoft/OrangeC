...
void thunkForImportTable(EXPRESSION** exp)
{
        ...
        ((*exp)->type == en_pc || !(*exp)->v.func->ascall))
    {
        ...
        if (!search)
        {
            SYMBOL* newThunk;
            char buf[2048];
            my_sprintf(buf, "@$%s", sym->sb->decoratedName);
...
void ValidateMSILFuncPtr(TYPE* dest, TYPE* src, EXPRESSION** exp)
{
    ...
    if (isfunction(dest))
    {
        ...
        managedDest =
            basetype(dest)->sp->sb->attribs.inheritable.linkage2 != lk_unmanaged && msilManaged(basetype(dest)->sp);
    }
...
static LEXEME* variableName(LEXEME* lex, SYMBOL* funcsp, TYPE* atp, TYPE** tp, EXPRESSION** exp, bool* ismutable, int flags)
{
    ...
    if (sym)
    {
        ...
        *tp = sym->tp;
        ...
        if (sym->sb && sym->sb->templateLevel && !isfunction(sym->tp) && sym->sb->storage_class != sc_type && !ismember(sym) && MATCHKW(lex, lt))
        {
            lex = getsym();
            ...
            if (sym->tp->type == bt_templateparam)
            {
                ...
                switch (sym->sb->storage_class)
                {
                    ...
                    case sc_localstatic:
                        ...
                        {
                            if (funcsp->sb->promotedToInline || cparams.prm_cplusplus)
                            {
        ...
        else
        {
            if (sym->tp->type == bt_templateparam)
            {
                if (*exp && (*exp)->type != en_packedempty && !sym->tp->templateParam->p->packed)
                {
                    if (sym->tp->templateParam->p->type == kw_int)
                        ...
                        *tp = &stdint;
                    if (sym->sb && sym->sb->templateLevel && sym->sb->storage_class != sc_type && !ismember(sym) && !isfunction(sym->tp))
                    {
            ...
            else
            {
                ...
                if (sym->sb->storage_class != sc_overloads)
                {
                    if (!isstructured(*tp) && basetype(*tp)->type != bt_memberptr && !isfunction(*tp) &&
                        sym->sb->storage_class != sc_constant && sym->sb->storage_class != sc_enumconstant && sym->tp->type != bt_void)
                    {
            ...
            if (cparams.prm_cplusplus && sym->sb && (sym->sb->storage_class == sc_member || sym->sb->storage_class == sc_mutable))
            {
...
static LEXEME* expression_member(LEXEME* lex, SYMBOL* funcsp, TYPE** tp, EXPRESSION** exp, bool* ismutable, int flags)
{
    ...
    if ((architecture == ARCHITECTURE_MSIL) && cparams.msilAllowExtensions &&
    ...
    if (MATCHKW(lex, pointsto))
    {
        ...
        } while ((cparams.prm_cplusplus || (architecture == ARCHITECTURE_MSIL)) &&
                ...
                *tp = sp2->tp;
                ...
                else
                {
                    ...
                    */
                    ...
                    {
                        EXPRESSION* offset;
                        if ((architecture == ARCHITECTURE_MSIL) && !isarray(sp2->tp))
        ...
        while (hr || list)
        {
            ...
            if (!hr || hr->p->tp->type != bt_any)
            {
                ...
                if (matching && hr)
                {
                    ...
                    {
                        hasEllipse = true;
                        vararg = (architecture == ARCHITECTURE_MSIL) && msilManaged(params->sp);
                ...
                if (!decl && !hasEllipse && (architecture == ARCHITECTURE_MSIL))
                    toolong = true;
                if (matching)
                {
                    if (!decl)
                    ...
                    else
                    {
                        ...
                        join:
                            ...
                            {
                                if ((architecture != ARCHITECTURE_MSIL) || !isstructured(decl->tp))
                                {
                ...
                {
                    if ((architecture != ARCHITECTURE_MSIL) || (!isstructured(dest) && (!isarray(dest) || !basetype(dest)->msil)))
            ...
            {
                bool ivararg = vararg;
                if (list->exp->type == en_auto && list->exp->v.sp->sb->va_typeof)
                    ivararg = false;
                list->vararg = ivararg;
                if (hr)
                {
                ...
                else if (isref(sym->tp))
                {
                    if (isstructured(basetype(sym->tp)->btp))
                    {
                        ...
                        if ((!isconst(basetype(sym->tp)->btp) && !isconst(sym->tp) &&
                             (sym->tp->type != bt_rref ||
                              (!func->sb->templateLevel && (!func->sb->parentClass || !func->sb->parentClass->sb->templateLevel) /*forward*/)) &&
                             isconst(tpx)) ||
...
LEXEME* expression_arguments(LEXEME* lex, SYMBOL* funcsp, TYPE** tp, EXPRESSION** exp, int flags)
{
    ...
    if ((architecture == ARCHITECTURE_MSIL) && funcparams->sp)
    {
    ...
    else if ((cparams.prm_cplusplus || (architecture == ARCHITECTURE_MSIL)) && funcparams->sp)
    {
        SYMBOL* sym = nullptr;
        // add this ptr
...
static EXPRESSION* nodeSizeof(TYPE* tp, EXPRESSION* exp, int flags)
{
    ...
    if (!exp)
    {
        ...
        if ((flags & _F_SIZEOF) && tp->type == bt_pointer && tp->array && exp_in && exp_in->type == en_l_p && exp_in->left->type == en_auto && exp_in->left->v.sp->sb->storage_class == sc_parameter)
...
static LEXEME* expression_ampersand(LEXEME* lex, SYMBOL* funcsp, TYPE* atp, TYPE** tp, EXPRESSION** exp, int flags)
{
    ...
    if (*tp)
    {
        ...
        if (exp1->type == en_void)
            exp1 = exp1->left;
        symRef = (architecture == ARCHITECTURE_MSIL) ? GetSymRef(exp1) : nullptr;
        btp = basetype(*tp);
...
static LEXEME* expression_add(LEXEME* lex, SYMBOL* funcsp, TYPE* atp, TYPE** tp, EXPRESSION** exp, bool* ismutable, int flags)
{
    ...
    while (MATCHKW(lex, plus) || MATCHKW(lex, minus))
    {
        ...
        else
        {
            ...
            LookupSingleAggregate(*tp, exp);
            LookupSingleAggregate(tp1, &exp1);
            if ((architecture == ARCHITECTURE_MSIL) && cparams.msilAllowExtensions && kw == plus &&
...
static LEXEME* expression_equality(LEXEME* lex, SYMBOL* funcsp, TYPE* atp, TYPE** tp, EXPRESSION** exp, bool* ismutable, int flags)
{
    ...
    while (MATCHKW(lex, eq) || MATCHKW(lex, neq))
    {
        ...
        else
        {
            ...
            if (isstructured(*tp) || isstructured(tp1))
            {
                if ((architecture != ARCHITECTURE_MSIL) || (!isconstzero(*tp, *exp) && !isconstzero(tp1, exp1)))
                    error(ERR_ILL_STRUCTURE_OPERATION);
            }
            else if (isvoid(*tp) || isvoid(tp1) || ismsil(*tp) || ismsil(tp1))
                error(ERR_NOT_AN_ALLOWED_TYPE);
    ...
    while (MATCHKW(lex, kw))
    {
        ...
        if (isstructured(*tp) || isstructured(tp1))
            ...
            error(ERR_NOT_AN_ALLOWED_TYPE);
                 ...
                 ((architecture != ARCHITECTURE_MSIL) || cparams.msilAllowExtensions))
            error(ERR_SCOPED_TYPE_MISMATCH);
...
LEXEME* expression_assign(LEXEME* lex, SYMBOL* funcsp, TYPE* atp, TYPE** tp, EXPRESSION** exp, bool* ismutable, int flags)
{
    ...
    while (!done && lex)
    {
        ...
        symRef = (architecture == ARCHITECTURE_MSIL) ? temp : nullptr;
        LookupSingleAggregate(tp1, &exp1);
        ...
        if (isconstraw(*tp, true) && !localMutable && (!temp || temp->v.sp->sb->storage_class != sc_parameter || !isarray(*tp)))
            ...
            switch (kw)
            {
                ...
                case assign:
                    ...
                    {
                        if (!((architecture == ARCHITECTURE_MSIL) && basetype(*tp)->sp->sb->msil &&

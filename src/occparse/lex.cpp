...
namespace Parser
{
    ...
    static bool kwmatches(KEYWORD* kw)
    {
        if (Optimizer::cparams.prm_assemble)
            ...
            return true;
                 ...
                 ((kw->matchFlags & KW_MSIL) && (Optimizer::architecture == ARCHITECTURE_MSIL) && Optimizer::cparams.msilAllowExtensions) ||
    ...
    e_lexType getNumber(const unsigned char** ptr, const unsigned char** end, unsigned char* suffix, FPF* rval, long long* ival)
    {
        ...
        while ((Optimizer::cparams.prm_cplusplus && **ptr == '\'') || radix36(**ptr) < radix || (Optimizer::cparams.prm_assemble && radix36(**ptr) < 16))
        {
            if (**ptr != '\'')
                *p++ = **ptr;
            (*ptr)++;

...
int getNumber(unsigned char **ptr, unsigned char **end, unsigned char *suffix, FPF *rval, LLONG_TYPE *ival)
{
    ...
    *suffix = 0;
    ...
    if (!floatradix && !hasdot)
    {
        ...
        else if (((cparams.prm_c99 || cparams.prm_cplusplus) && !stricmp((char *)suffix, "LL")) || (!cparams.prm_ansi && !stricmp((char *)suffix, "i64")))
        {
            lastst = l_ll;
            suffix[0] = 0;
        }
        else if (((cparams.prm_c99 || cparams.prm_cplusplus) && (!stricmp((char *)suffix, "ULL") || !stricmp((char *)suffix, "LLU"))) || (!cparams.prm_ansi && !stricmp((char *)suffix, "ui64")))
        {
            lastst = l_ull;
            suffix[0] = 0;
        }

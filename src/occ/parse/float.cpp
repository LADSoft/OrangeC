            ...
            */
            ...
            if (locx.sign ^ locy.sign ^ operation)
            {
                ...
                */
                borrow = 0;
                for (i = (INTERNAL_FPF_PRECISION - 1); i >= 0; i--)
                    Sub16Bits(&borrow, &z->mantissa[i], locx.mantissa[i], locy.mantissa[i]);
            ...
            else
            {
                ...
                for (i = (INTERNAL_FPF_PRECISION - 1); i >= 0; i--)
                {
                    Add16Bits(&carry, &z->mantissa[i], locx.mantissa[i], locy.mantissa[i]);
                }
...
int LongDoubleToFPF(FPFC* dest, unsigned char* src)
{
    ...
    {
        if (dest->mantissa[0] == 0x8000 && (dest->mantissa[1] | dest->mantissa[2] | dest->mantissa[3]) == 0)
            dest->type = IFPF_IS_INFINITY;
        else
            dest->type = IFPF_IS_NAN;
        dest->exp = exp;

                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            tn = sp->tp;
                            if (!templateNestingCount && isstructured(tn) && basetype(tn)->sp->templateLevel &&
                                !basetype(tn)->sp->instantiated)
                            {
...
exit:
    if ((architecture == ARCHITECTURE_MSIL) && cparams.msilAllowExtensions && tn)
    {
...
LEXEME* getFunctionParams(LEXEME* lex, SYMBOL* funcsp, SYMBOL** spin, TYPE** tp, bool inTemplate, enum e_sc storage_class)
{
    ...
    else if (!cparams.prm_cplusplus && !(architecture == ARCHITECTURE_MSIL) && ISID(lex))
    {
        SYMBOL* spo = nullptr;
        sp->oldstyle = true;
    ...
    else if (MATCHKW(lex, ellipse))
    {
        ...
        skip(&lex, closepa);
             ...
             ((architecture == ARCHITECTURE_MSIL) && cparams.msilAllowExtensions && !MATCHKW(lex, closepa) && *spin))
    {
        // () is a function
        if (MATCHKW(lex, closepa))
        {
                      ...
                      bool asFriend, int consdest, bool beforeOnly, bool funcptr)
{
    ...
    if ((architecture == ARCHITECTURE_MSIL) && cparams.msilAllowExtensions && MATCHKW(lex, openbr))
    {
        // managed array
        bool isvla = false;
        TYPE* quals = nullptr;
...
static void allocateVLA(LEXEME* lex, SYMBOL* sp, SYMBOL* funcsp, BLOCKDATA* block, TYPE* btp, bool bypointer)
{
    ...
    if (result != nullptr)
    {
        ...
        if (sp->storage_class != sc_typedef && !bypointer)
        {
            ...
            *rptr = (architecture == ARCHITECTURE_MSIL) ? ep1 : exprNode(en_void, ep1, nullptr);
            sp->assigned = true;
        }
        st->select = result;
                ...
                do
                {
                    ...
                    if (lex)
                    {
                        ...
                        if (basetype(sp->tp)->type == bt_func)
                        {
                            ...
                            if (sp->storage_class == sc_static && (architecture == ARCHITECTURE_MSIL))
                            {
                                if (!sp->label)
                                    sp->label = nextLabel++;
                            }
                            ...
                            else
                            {
                                ...
                                else if (sp->parentClass && !sp->templateParams && !(architecture == ARCHITECTURE_MSIL))

...
LEXLIST* expression_func_type_cast(LEXLIST* lex, SYMBOL* funcsp, TYPE** tp, EXPRESSION** exp, int flags)
{
    ...
    else
    {
        *tp = nullptr;
        ...
        if ((!templateNestingCount || instantiatingTemplate) && isstructured(*tp) && basetype(*tp)->sp->name[0] == '_' && basetype(*tp)->sp->name[1] == '_')
        {
            if (basetype(*tp)->sp->name[2] == 'm' && !strcmp(basetype(*tp)->sp->name, "__make_integer_seq"))
            {
                auto sym = TemplateClassInstantiateInternal(basetype(*tp)->sp, basetype(*tp)->sp->templateParams->next, false);
                if (sym)
                    *tp = sym->tp;
            }
        }

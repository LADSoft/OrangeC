...
#if !defined (_STLP_DONT_SUP_DFLT_PARAM)
  explicit map(initializer_list<value_type> __lst, const allocator_type& __a = allocator_type())
#else
  map(initializer_list<value_type> __lst)
       : _M_t(_Compare(), allocator_type()) {
            for (auto __el :__lst)
                (*this)[__el.first] = __el.second;
        }
  map(initializer_list<value_type> __lst, const allocator_type& __a)
#endif
       : _M_t(_Compare(), __a) {
            for (auto __el :__lst)
                (*this)[__el.first] = __el.second;

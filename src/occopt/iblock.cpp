...
static QUAD* add_dag(QUAD* newQuad)
{
    ...
    */
    ...
    {
        if (newQuad->ans && (newQuad->ans->mode == i_ind || newQuad->ans->offset->type != se_tempref))
            flush_dag();
                 ...
                 && ((cparams.prm_optimize_for_speed || cparams.prm_optimize_for_size) &&
                     ((newQuad->ans->offset->type == se_tempref && newQuad->ans->mode == i_direct) || node->dc.opcode == i_icon ||
...
void InsertInstruction(QUAD* before, QUAD* ins)
{
    ...
    {
        ins->temps = 0;
        if (ins->ans && ((ins->ans->offset && ins->ans->offset->type == se_tempref) || ins->ans->offset2))
        {
            ins->temps |= TEMP_ANS;
            if (ins->ans->mode == i_direct)
            {
        ...
        if (ins->dc.left && ((ins->dc.left->offset && ins->dc.left->offset->type == se_tempref) || ins->dc.left->offset2))
        {
        ...
        if (ins->dc.right && ins->dc.right->offset && ins->dc.right->offset->type == se_tempref)
        {

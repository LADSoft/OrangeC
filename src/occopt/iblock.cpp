...
namespace Optimizer
{
    ...
    static Optimizer::QUAD* add_dag(Optimizer::QUAD* newQuad)
    {
        ...
        {
            if ((cparams.prm_optimize_for_speed || cparams.prm_optimize_for_size) && !(Optimizer::chosenAssembler->arch->denyopts & DO_NOLOCAL))
            {
                ...
                if (newQuad->ans && (newQuad->dc.opcode != i_assn || newQuad->dc.left->mode != i_immed) &&
                    ...
                    (newQuad->ans->size < ISZ_FLOAT || Optimizer::chosenAssembler->arch->hasFloatRegs) &&
                        ...
                        (!newQuad->dc.left->vol && (newQuad->dc.left->size < ISZ_FLOAT || Optimizer::chosenAssembler->arch->hasFloatRegs))) &&
                            (!newQuad->dc.right || !(newQuad->livein & IM_LIVERIGHT) ||
                        (!newQuad->dc.right->vol && (newQuad->dc.right->size < ISZ_FLOAT || Optimizer::chosenAssembler->arch->hasFloatRegs)))))
...

        {
            if (newQuad->ans && (newQuad->ans->mode == i_ind || newQuad->ans->offset->type != Optimizer::se_tempref))
                ...
                ((newQuad->ans->offset->type == Optimizer::se_tempref && newQuad->ans->mode == i_direct) || node->dc.opcode == i_icon ||
    ...
    Optimizer::QUAD* gen_icode_with_conflict(enum i_ops op, Optimizer::IMODE* res, Optimizer::IMODE* left, Optimizer::IMODE* right, bool conflicting)
        /*
    ...
    void InsertInstruction(Optimizer::QUAD* before, Optimizer::QUAD* ins)
    {
        ...
        else
        {
            ...
            if (ins->dc.left && ((ins->dc.left->offset && ins->dc.left->offset->type == Optimizer::se_tempref) || ins->dc.left->offset2))
            {

...
namespace Parser
{
    ...
    {
        Optimizer::globalCache.push_back(Optimizer::SymbolManager::Get(sp));
    }
    void WeedExterns()
    {
        for (auto it = Optimizer::externals.begin(); it != Optimizer::externals.end();)
        {
            ...
            if (!sym->ispure &&
                ...
                (!sym->initialized &&
                    (sym->tp->type == Optimizer::st_func || (sym->tp->type != Optimizer::st_func && sym->storage_class != Optimizer::scc_global &&
    ...
    static bool validateAnonymousUnion(SYMBOL* parent, TYPE* unionType)
    {
        ...
        unionType = basetype(unionType);
        {
            ...
            while (newhr)
            {
                ...
                else if ((Optimizer::cparams.prm_cplusplus && member->sb->storage_class == sc_type) || member->sb->storage_class == sc_typedef)
                {
                    error(ERR_ANONYMOUS_UNION_NO_FUNCTION_OR_TYPE);
                    rv = false;
                }
                ...
                else if (Optimizer::cparams.prm_cplusplus && member->sb->storage_class != sc_member && member->sb->storage_class != sc_mutable)
                {
    ...
    static LEXEME* getStorageClass(LEXEME* lex, SYMBOL* funcsp, enum e_sc* storage_class, enum e_lk* linkage, Optimizer::ADDRESS* address,
    ...
    LEXEME* getFunctionParams(LEXEME* lex, SYMBOL* funcsp, SYMBOL** spin, TYPE** tp, bool inTemplate, enum e_sc storage_class)
    {
        ...
        else if (MATCHKW(lex, ellipse))
        {
            ...
            skip(&lex, closepa);
                 ...
                 ((Optimizer::architecture == ARCHITECTURE_MSIL) && Optimizer::cparams.msilAllowExtensions && !MATCHKW(lex, closepa) && *spin))
        {
            // () is a function
            if (MATCHKW(lex, closepa))
            {
                          ...
                          bool asFriend, int consdest, bool beforeOnly, bool funcptr)
    {
        ...
        if (ISID(lex) || MATCHKW(lex, classsel) || MATCHKW(lex, kw_operator) || (Optimizer::cparams.prm_cplusplus && MATCHKW(lex, ellipse)))
        {
            ...
            switch (KW(lex))
            {
                ...
                case land:
                    ...
                    if (Optimizer::cparams.prm_cplusplus || ((Optimizer::architecture == ARCHITECTURE_MSIL) && Optimizer::cparams.msilAllowExtensions &&
                ...
                else
                {
                    ...
                    do
                    {
                        ...
                        if (!sp)
                        {
                            ...
                            tp1 = basetype(tp1);
                            if (Optimizer::cparams.prm_cplusplus && storage_class_in == sc_global && tp1->type == bt_union && tp1->sp->sb->anonymous)
                            {
                            ...
                            */
                            ...
                            else if (spi)
                            {
                                ...
                                if (isfunction(spi->tp))
                                {
                                    ...
                                    if (Optimizer::cparams.prm_cplusplus && (MATCHKW(lex, begin) || MATCHKW(lex, colon) || MATCHKW(lex, kw_try)))
                                    {
                        ...
                        if (lex)
                        {
                            ...
                            if (basetype(sp->tp)->type == bt_func)
                            {
                                ...
                                else
                                {
                                    ...
                                    else if (sp->sb->parentClass && !sp->templateParams && !(Optimizer::architecture == ARCHITECTURE_MSIL))
                            ...
                            else
                            {
                                ...
                                if (!sp->sb->label && (sp->sb->storage_class == sc_static || sp->sb->storage_class == sc_localstatic) && (Optimizer::architecture == ARCHITECTURE_MSIL))
                                    sp->sb->label = Optimizer::nextLabel++;
                                if (Optimizer::cparams.prm_cplusplus && sp->sb->storage_class != sc_type && sp->sb->storage_class != sc_typedef &&

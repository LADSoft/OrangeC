...
int ccWriteLineNumbers(char* symname, char* typename, char* filename, int indirectCount, sqlite_int64 struct_id,
                       sqlite_int64 main_id, int start, int end, int altend, int flags, sqlite_int64* id)
{
    int rc = SQLITE_ERROR;
    sqlite3_int64 sym_id, type_id, file_id;
    if (ccWriteName(symname, &sym_id, main_id) && ccWriteName(typename, &type_id, main_id) && ccWriteFileName(filename, &file_id))
    {
        static char* query =
...
int ccWriteStructField(sqlite3_int64 name_id, char* symname, char* typename, int indirectCount, sqlite3_int64 struct_id,
                       sqlite3_int64 file_id, sqlite_int64 main_id, int flags, int* order, sqlite_int64* id)
{
    int rc = SQLITE_ERROR;
    sqlite3_int64 sym_id, type_id;
    if (ccWriteMemberName(symname, &sym_id) && ccWriteName(typename, &type_id, main_id))
    {
        static char* query =

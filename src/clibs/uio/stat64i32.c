...
int _RTL_FUNC _wstat64i32(wchar_t* path, struct _stat64i32* __statbuf)
{
    int fd, rv, rootdir;
    char pbuf[265];
...
int _RTL_FUNC _stat64i32(char* path, struct _stat64i32* __statbuf) 
{
    wchar_t buf[256], *q = buf;
    while (*path)
        *q++ = *path++;
    *q = *path;
    return _wstat64i32(buf, __statbuf);
}

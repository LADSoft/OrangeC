...
bool Importer::EnterNamespace(const Namespace *nameSpace)
{
    ...
    level_++;
    SYMLIST **hr = LookupName((char *)nameSpace->Name().c_str(), nameSpaces_.size() == 0 ? globalNameSpace->valueData->syms : nameSpaces_.back()->nameSpaceValues->valueData->syms);
    SYMBOL *sp;
    if (!hr)
    {
    ...
    if (nameSpaces_.size())
    {
        ...
        if (!sp)
        {
            ...
            if (useGlobal())
                ...
                insert(sp, structures_.size() ? structures_.back()->tp->syms : nameSpaces_.back()->nameSpaceValues->valueData->syms);
            sp->msil = (void *)cls;

...
static UBYTE segoverxlattab[] = {0x3e, 0x2e, 0x3e, 0x26, 0x64, 0x65, 0x36};
static UBYTE segtab[] = {0x18, 0x8, 0x18, 0x0, 0x20, 0x28, 0x10};
...
OPCODE popn_call[] = {
    {OP_BRANCH, 0x00e8, 0x0000}, {OP_RM, 0x10ff, 0x0200}, {OP_SEGBRANCH, 0x009a, 0x0000}, {OP_FARRM, 0x18ff, 0x0000}, {0, 0, 0},
};
OPCODE popn_cbw[] = {
...
OPCODE popn_jmp[] = {
    {OP_RM, 0x20ff, 0x0000},        {OP_BRANCH, 0x00e9, 0x0000}, {OP_SHORTBRANCH, 0x00eb, 0x0000},
    {OP_SEGBRANCH, 0x00ea, 0x0000}, {OP_FARRM, 0x28ff, 0x0000},  {0, 0, 0},
};
OPCODE popn_jna[] = {
    {OP_BRANCH, 0x0086, 0x0803},
    {OP_SHORTBRANCH, 0x0076, 0x0000},
...
OPCODE popn_mov[] = {
    {OP_REG03, 0x00b8, 0x0000},    {OP_ABSACC, 0x00a2, 0x0000},   {OP_ACCABS, 0x00a0, 0x0000},
    {OP_ACCIMMB3, 0x00b0, 0x0000}, {OP_REGRMREG, 0x0088, 0x0200}, {OP_RMIMM, 0x00c6, 0x0200},
    {OP_386REG, 0xc020, 0x0b03},   {OP_SEGRMSEG, 0x008c, 0x0200}, {0, 0, 0},
};
OPCODE popn_movs[] = {
...
OPCODE popn_pop[] = {
    {OP_WREG02, 0x0058, 0x0000}, {OP_PUSHW, 0x008f, 0x0000}, {OP_SEG35, 0x0007, 0x0000}, {OP_SEG35, 0x00a1, 0x0803}, {0, 0, 0},
};
OPCODE popn_popa[] = {
...
OPCODE popn_push[] = {
    {OP_WREG02, 0x0050, 0x0000}, {OP_IMM, 0x0068, 0x0002},   {OP_PUSHW, 0x30ff, 0x0000},
    {OP_SEG35, 0x0006, 0x0000},  {OP_SEG35, 0x00a0, 0x0803}, {0, 0, 0},
};
OPCODE popn_pusha[] = {
...
#define NOP 0x90
    ...
    if (beGetCurrentFunc)
        if (oa_currentSeg == virtseg)
            ...
            beGetCurrentFunc->offset = outcode_base_address = segs[codeseg].curbase;
    else
...
int LabelSeg(int lbl) { return labelbuf[lbl / 512][lbl % 512].seg; }
/*-------------------------------------------------------------------------*/
...
void InsertLine(int address, LINEDATA* linedata) { InsertAttrib(address, e_ad_linedata, linedata, 0); }
void InsertVarStart(int address, SYMBOL* sp) { InsertAttrib(address, e_ad_vardata, sp, 0); }
void InsertFunc(int address, SYMBOL* sp, int start) { InsertAttrib(address, e_ad_funcdata, sp, start); }
void InsertBlock(int address, int start) { InsertAttrib(address, e_ad_blockdata, NULL, start); }
...
void outcode_genref(SYMBOL* sp, int offset)
{
    ...
    gen_symbol_fixup(fm_symbol, oa_currentSeg, seg->last->address + seg->last->lastfilled, sp);
}
...
void outcode_gen_labref(int n)
{
    ...
    gen_label_fixup(fm_label, oa_currentSeg, seg->last->address + seg->last->lastfilled, n);
}
...
void outcode_genlong(int val) { emit(oa_currentSeg, &val, 4); }
...
void outcode_end_virtual_seg(SYMBOL* sp) { (void)sp; }
...
int asmrm(int reg, OCODE* ins, AMODE* data, UBYTE** p)
{
    ...
    switch (data->mode)
    {
        ...
        case am_indispscale:
            ...
            {
                if (data->sreg == ESP || (data->sreg == data->preg && data->sreg == EBP))
                    return 0;
                if (data->preg != EBP)
                {
                    *(*p)++ = 4 + reg;
                    *(*p)++ = data->preg + (data->sreg << 3) + (data->scale << 6);
                    ins->addroffset = *p - ins->outbuf;
                    val = resolveoffset(ins, data->offset, &resolved);
#ifdef FULLVERSION
                    if (!val && resolved)
        ...
        else
        {
            ...
            *(*p)++ = data->oper2->offset->v.i;
            return 1;
             ...
             (data->oper2->length == ISZ_UCHAR || data->oper2->length == -ISZ_UCHAR || data->oper2->length == ISZ_BOOLEAN) &&
             data->oper2->preg == ECX)
    {
        val |= 0x12;
    }
...
int AOP10(OPCODE* descript, OCODE* data, UBYTE** p)
{
    ...
    if (data->oper1->length > ISZ_ADDR || data->oper1->length == ISZ_ULONGLONG)
        return 0;
    if (data->oper1->mode != am_immed || data->oper1->offset->type != en_labcon)
        return 0;
    *(*p)++ = descript->ocvalue;
...
int AOP14(OPCODE* descript, OCODE* data, UBYTE** p)
{
    ...
    if (data->oper1->mode == am_dreg && data->oper1->preg == EAX && data->oper2->mode == am_immed &&
        (data->oper1->length == ISZ_UCHAR || data->oper1->length == -1 || data->oper1->length == ISZ_BOOLEAN ||
         data->oper2->length == ISZ_USHORT || data->oper2->length == -ISZ_USHORT || data->oper2->length == ISZ_U16 ||
...
int AOP30(OPCODE* descript, OCODE* data, UBYTE** p)
{
    ...
    if (data->oper1->length != ISZ_UCHAR && data->oper1->length != -ISZ_UCHAR && data->oper1->length != ISZ_BOOLEAN)
        val |= 1;
    if (data->oper2->mode == am_dreg && data->oper2->preg == ECX &&
        (data->oper2->length == ISZ_UCHAR || data->oper2->length == -ISZ_UCHAR || data->oper2->length == ISZ_BOOLEAN))
    {
...
int AOP33(OPCODE* descript, OCODE* data, UBYTE** p)
{
    ...
    if (data->oper2->length == ISZ_USHORT || data->oper2->length == -ISZ_USHORT || data->oper2->length == ISZ_U16)
        *(*p)++ = 0x66;
    if (data->oper3->mode == am_dreg && data->oper3->preg == ECX &&
        (data->oper3->length == ISZ_UCHAR || data->oper3->length == -ISZ_UCHAR || data->oper3->length == ISZ_BOOLEAN))
    {
        *(*p)++ = val | 1;
...
int AOP36(OPCODE* descript, OCODE* data, UBYTE** p)
{
    ...
    if (data->oper2)
    {
        if (data->oper1->mode != am_freg || data->oper2->mode != am_freg || data->oper2->preg != 0)
            return 0;
    }
    else if (!data->oper1)
        val = 0x100;
 ...
 *  same as aop40
 */
int AOP45(OPCODE* descript, OCODE* data, UBYTE** p) { return AOP40(descript, data, p); }
...
int AOP52(OPCODE* descript, OCODE* data, UBYTE** p)
{
    ...
    if (data->oper1->length == ISZ_LDOUBLE || data->oper1->length == ISZ_ILDOUBLE)
        return 0;
    if (data->oper1->length != ISZ_UINT && data->oper1->length != -ISZ_UINT && data->oper1->length != ISZ_U32 &&
        data->oper1->length != ISZ_ULONGLONG && data->oper1->length != -ISZ_ULONGLONG)
        val |= 4;
    if (data->oper1->length == ISZ_DOUBLE || data->oper1->length == ISZ_IDOUBLE)
    {
        val |= 0x2000;
 ...
 */
int AOP61(OPCODE* descript, OCODE* data, UBYTE** p) { return AOP58(descript, data, p); }
 ...
 * stos	byte ptr [edi]
 */
int AOP64(OPCODE* descript, OCODE* data, UBYTE** p) { return AOP63(descript, data, p); }
...
int (*genfunctab[])(OPCODE* descript, OCODE* data, UBYTE** p) = {
    AOP0,  AOP1,  AOP2,  AOP3,  AOP4,  AOP5,  AOP6,  AOP7,  AOP8,  AOP9,  AOP10, AOP11, AOP12, AOP13, AOP14, AOP15, AOP16,
    AOP17, AOP18, AOP19, AOP20, AOP21, AOP22, AOP23, AOP24, AOP25, AOP26, AOP27, AOP28, AOP29, AOP30, AOP31, AOP32, AOP33,
    AOP34, AOP35, AOP36, AOP37, AOP38, AOP39, AOP40, AOP41, AOP42, AOP43, AOP44, AOP45, AOP46, AOP47, AOP48, AOP49, AOP50,
    AOP51, AOP52, AOP53, AOP54, AOP55, AOP56, AOP57, AOP58, AOP59, AOP60, AOP61, AOP62, AOP63, AOP64, AOP65,
};
int outcode_AssembleIns(OCODE* ins, int address)
{
    ...
    if (ins->opcode >= op_aaa)
    {
        ...
        {
            p = ins->outbuf;
            if (ins->oper1 && ins->oper1->seg &&
                (ins->oper1->mode == am_direct || ins->oper1->mode == am_indisp || ins->oper1->mode == am_indispscale))
                *p++ = segoverxlattab[ins->oper1->seg];
            else if (ins->oper2 && ins->oper2->seg &&
                     (ins->oper2->mode == am_direct || ins->oper2->mode == am_indisp || ins->oper2->mode == am_indispscale))
                *p++ = segoverxlattab[ins->oper2->seg];
            else if (ins->oper3 && ins->oper3->seg &&
                     (ins->oper3->mode == am_direct || ins->oper3->mode == am_indisp || ins->oper3->mode == am_indispscale))
...
void outcode_optimize(OCODE* peeplist)
{
    ...
    while (!done)
    {
        ...
        {
            head->address += offset;
            if ((head->branched & BR_LONG) && head->oper1->offset->type == en_labcon)
            {
    ...
    while (peeplist)
    {
        ...
        if (!peeplist->resolved && !resolved)
        {
            ...
            if (oper->offset->type == en_sub && oper->offset->left->type == en_threadlocal)
            {
                ...
                gen_threadlocal_fixup(oa_currentSeg, peeplist->address + peeplist->addroffset, node->v.sp, node1->v.sp);
            }
            else
            {
                ...
                if (!node)
                    ...
                    gen_label_fixup(peeplist->branched ? fm_rellabel : fm_label, oa_currentSeg,
                                    peeplist->address + peeplist->addroffset, node->v.i);
                else
                    gen_symbol_fixup(peeplist->branched ? fm_relsymbol : fm_symbol, oa_currentSeg,
                                     peeplist->address + peeplist->addroffset, node->v.sp);
            }
            ...
            if (peeplist->resobyte == 3)
            {
                ...
                if (!node)
                    ...
                    gen_label_fixup(peeplist->branched ? fm_rellabel : fm_label, oa_currentSeg,
                                    peeplist->address + peeplist->addroffset + 4, node->v.i);
                else
                    gen_symbol_fixup(peeplist->branched ? fm_relsymbol : fm_symbol, oa_currentSeg,
                                     peeplist->address + peeplist->addroffset + 4, node->v.sp);
            }
        }

...
bool NetLinkMain::CreateExecutable(CmdFiles& files)
{
    ...
    if (netCore)
    {
        return netCore->DumpOutputFile(GetOutputFile(files).c_str(), AssemblyFile.GetValue()  ? PELib::ilasm
...
int NetLinkMain::Run(int argc, char** argv)
{
    ...
    {
        netCore = new NetCore(PELib::bits32, GUIApp.GetValue(),
                              NetCoreSwitch.GetValue() ? NetCoreSwitch.GetValue() : NetCore::DummyChooseLatest);

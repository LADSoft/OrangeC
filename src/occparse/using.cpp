...
void insertUsing(SYMBOL** sp_out, AccessLevel access, StorageClass storage_class, bool inTemplate, bool hasAttributes)
{
    SYMBOL* sp;
            ...
            if (MATCHKW(Keyword::assign_))
            {
                ...
                if (inTemplate && (ISID() || MATCHKW(Keyword::classsel_) || MATCHKW(Keyword::typename_)))
                {
                    ...
                    if (!isTemplate)
                    {
                        ...
                        tp =
                            TypeGenerator::BeforeName(nullptr, tp, &sp, nullptr, nullptr, false, storage_class, &linkage, &linkage2,
            ...
            for (; it != path.end();)
            {
                ...
                if (sp)
                {
                    ...
                    else if (sp->tp->IsStructured())
                    {
                        ...
                        {
                            std::list<TEMPLATEPARAMPAIR>* lst = nullptr;
                            it->second.Replay([&]() { GetTemplateArguments(theCurrentFunc, sp, &lst); });
                            SYMBOL* sp1;

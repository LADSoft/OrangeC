...
bool ismember(SYMBOL* sym)
{
    ...
    EXPRESSION* convertInitToExpression(TYPE* tp, SYMBOL* sym, EXPRESSION *expsym, SYMBOL* funcsp, INITIALIZER* init, EXPRESSION* thisptr, bool isdest)
    {
        ...
        while (init)
        {
            ...
            if (init->basetp)
            {
                ...
                else if (isstructured(init->basetp) || isarray(init->basetp))
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if (expsym)
                            {
                                if (Optimizer::cparams.prm_cplusplus && isstructured(init->basetp) && !init->basetp->sp->sb->trivialCons)
                                {
                                    TYPE* ctype = init->basetp;
                ...
                else
                {
                    ...
                    else if (init->offset || (init->next && init->next->basetp && (Optimizer::chosenAssembler->arch->denyopts & DO_UNIQUEIND)))
        ...
        if ((sym || expsymin) && !noClear && !isdest &&
            (isarray(tp) ||
             (isstructured(tp) && ((!Optimizer::cparams.prm_cplusplus && (Optimizer::architecture != ARCHITECTURE_MSIL)) || !basetype(tp)->sp->sb->hasUserCons))))
        {

...
//            arg->exp = intNode(ExpressionNode::c_i, 1);
            (*lptr)->push_back(arg);
        }
        else
        {
...
static int FindBaseClassWithData(SYMBOL* sym, SYMBOL** result)
{
    ...
    {
        if (sym1->sb->storage_class == StorageClass::mutable_ || sym1->sb->storage_class == StorageClass::member)
        {
            if (result)
                *result = sym1;
            return n + 1;
...
static bool trivialCopyConstructible(TYPE* tp, bool rref)
{
    if (isstructured(tp))
    {
        ...
        {
            if (sym->sb->storage_class == StorageClass::mutable_ || sym->sb->storage_class == StorageClass::member)
...
static bool trivialAssignable(TYPE* tp, bool rref)
{
    ...
    {
        SYMBOL* ovl;
        ovl = search(basetype(tp)->syms, overloadNameTab[(int)Keyword::_assign - (int)Keyword::_new + CI_NEW]);
        if (ovl)
        {
            if (!trivialFunc(ovl, rref))
                return false;
...
static bool is_abstract(LEXLIST** lex, SYMBOL* funcsp, SYMBOL* sym, TYPE** tp, EXPRESSION** exp)
{
    ...
    *exp = intNode(ExpressionNode::c_i, rv);
    *tp = &stdint;
    return true;
}
static bool is_base_of(LEXLIST** lex, SYMBOL* funcsp, SYMBOL* sym, TYPE** tp, EXPRESSION** exp)
...
static bool is_class(LEXLIST** lex, SYMBOL* funcsp, SYMBOL* sym, TYPE** tp, EXPRESSION** exp)
{
    ...
    *lex = getTypeList(*lex, funcsp, &funcparams.arguments);
    if (funcparams.arguments->size() == 1)
    {
        rv = isstructured(first(funcparams.arguments)->tp) && basetype(first(funcparams.arguments)->tp)->type != BasicType::union_;
    }
    *exp = intNode(ExpressionNode::c_i, rv);
    *tp = &stdint;
    return true;
    ...
    *lex = getTypeList(*lex, funcsp, &funcparams.arguments, true);
    if (funcparams.arguments && funcparams.arguments->size())
    {
        ...
        if (tp2)
        {
            ...
            else if (isfuncptr(tp2))
            {
                if (funcparams.arguments->size() == 2)
                {
                    ...
                    else if (isstructured(tp3))
                    {
                        ...
                        funcparams.thisptr = intNode(ExpressionNode::c_i, 0);
                        funcparams.thistp = MakeType(BasicType::pointer, basetype(tp3));
                        funcparams.ascall = true;
            ...
            else if (basetype(tp2)->type == BasicType::memberptr)
            {
                if (funcparams.arguments->size() == 2)
                {
                    TYPE* tp3 = second(funcparams.arguments)->tp;
                    if (isref(tp3))
                        tp3 = basetype(basetype(tp3)->btp);
                    if (tp3->type == BasicType::memberptr)
                    {
                        tp2 = basetype(tp2);
                        SYMBOL* s1 = tp2->sp;
                        SYMBOL* s2 = tp3->sp;
            ...
            else if (isarithmetic(tp2) || ispointer(tp2) || basetype(tp2)->type == BasicType::enum_)
            {
                if (funcparams.arguments->size() == 1)
                {
                    rv = true;
            ...
            else if (isstructured(tp2))
            {
...
#endif
                {
                    ...
                    {
                        funcparams.thisptr = intNode(ExpressionNode::c_i, 0);
                        funcparams.thistp = MakeType(BasicType::pointer, basetype(tp2));
                        funcparams.ascall = true;
                        ...
                        for (auto arg : *funcparams.arguments)
                        {
                            while (arg->tp->type == BasicType::typedef_)
                                arg->tp = arg->tp->btp;
                            bool rref = arg->tp->type == BasicType::rref;
                            if (isref(arg->tp) && !isstructured(basetype(arg->tp)->btp))
                                arg->tp = basetype(arg->tp)->btp;
                            holdl.push_back(arg->tp->lref);
                            holdr.push_back(arg->tp->rref);
    ...
    *lex = getTypeList(*lex, funcsp, &funcparams.arguments);
    if (funcparams.arguments->size() == 2)
    {
        ...
        if (isref(from))
        {
            if (isref(to))
            {
                if (basetype(to)->type == BasicType::lref)
                    if (basetype(from)->type == BasicType::rref)
                        rv = false;
            }
        }
        if (isfunction(from))
        ...
        if (rv)
        {
            ...
            if (to->type == BasicType::templateparam)
                to = to->templateParam->second->byClass.val;
            if (from->type == BasicType::templateparam)
...
static bool is_enum(LEXLIST** lex, SYMBOL* funcsp, SYMBOL* sym, TYPE** tp, EXPRESSION** exp)
{
    ...
    *lex = getTypeList(*lex, funcsp, &funcparams.arguments);
    if (funcparams.arguments->size() == 1)
    {
        rv = basetype(first(funcparams.arguments)->tp)->type == BasicType::enum_;
    }
    *exp = intNode(ExpressionNode::c_i, rv);
    *tp = &stdint;
    return true;
    ...
    if (funcparams.arguments && funcparams.arguments->size() && isstructured(first(funcparams.arguments)->tp))
    {
        if (funcparams.arguments->size() == 2)
        {
           ...
           if (comparetypes(tp1, first(funcparams.arguments)->tp, true) || sameTemplate(tp1, first(funcparams.arguments)->tp))
              rv = trivialAssignable(first(funcparams.arguments)->tp, basetype(second(funcparams.arguments)->tp)->type== BasicType::rref);
        }
    }
    *exp = intNode(ExpressionNode::c_i, rv);
    *tp = &stdint;
    ...
    if (funcparams.arguments && funcparams.arguments->size() && isstructured(first(funcparams.arguments)->tp))
    {
        ...
        else if (funcparams.arguments->size() == 2)
        {
           ...
           if (isref(tp1))
               tp1 = basetype(tp1)->btp;
           if (comparetypes(tp1, first(funcparams.arguments)->tp, true) || sameTemplate(tp1, first(funcparams.arguments)->tp))
              rv = trivialCopyConstructible(first(funcparams.arguments)->tp, basetype(second(funcparams.arguments)->tp)->type== BasicType::rref);
        }
    }
    *exp = intNode(ExpressionNode::c_i, rv);
    *tp = &stdint;
...
static bool is_union(LEXLIST** lex, SYMBOL* funcsp, SYMBOL* sym, TYPE** tp, EXPRESSION** exp)
{
    ...
    *lex = getTypeList(*lex, funcsp, &funcparams.arguments);
    if (funcparams.arguments->size() == 1)
    {
        rv = basetype(first(funcparams.arguments)->tp)->type == BasicType::union_;
    }
    *exp = intNode(ExpressionNode::c_i, rv);
    *tp = &stdint;
    return true;
    ...
    if (isstructured(tp))
    {
        if (trivialDestructor(tp))
        {
            if (basetype(tp)->sp->sb->trivialCons || hasConstexprConstructor(tp))
            {
                ...
                if (basetype(tp)->type == BasicType::union_ && one)
                    return true;
                else if (all)
                    return true;
            }
    ...
    *lex = getTypeList(*lex, funcsp, &funcparams.arguments);
    if (funcparams.arguments->size() == 1)
    {
        *tp = first(funcparams.arguments)->tp;
        if (basetype(*tp)->type == BasicType::enum_)
            *tp = basetype(*tp)->btp;
    }
    else
    {
...
static SYMBOL* MakeIntegerSeqType(SYMBOL* sp, std::list<TEMPLATEPARAMPAIR>* args)
{
    ...
    if (e && isintconst(e))
    {
        ...
        int nt = (int)basetype(it->second->byClass.dflt)->type + e->v.i;
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            second->type = Keyword::_int;
            second->byNonType.tp = it->second->byClass.dflt;
            second->byNonType.val = intNode(ExpressionNode::c_i, i);
...
static TYPE* TypePackElementType(SYMBOL* sym, std::list<TEMPLATEPARAMPAIR>* args)
{
    ...
    if (e && isintconst(e))
    {
        ...
        if (it->second->packed)
        {
            ...
            ite = it->second->byPack.pack->end();
            it = it->second->byPack.pack->begin();
            if (it != ite && it->second->type == Keyword::_new)
                ++it;        
        }
        while (n-- && it != ite)
        {
        ...
        if (it == ite)
            return &stdany;
        return MakeType(BasicType::derivedfromtemplate, it->second->byClass.val ? it->second->byClass.val : it->second->byClass.dflt);
    }
    else
    {
        return &stdany;
...
SYMBOL* TypePackElementCls(SYMBOL* sym, std::list<TEMPLATEPARAMPAIR>* args)
{
    ...
    auto tp1 = MakeType(BasicType::typedef_, TypePackElementType(sym, args));

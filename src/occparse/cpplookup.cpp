...
namespace Parser
{
    ...
    static Optimizer::LIST* namespacesearchone(const char* name, NAMESPACEVALUELIST* ns, Optimizer::LIST* gather, bool tagsOnly, bool allowUsing)
    {
    ...
    static Optimizer::LIST* namespacesearchInternal(const char* name, NAMESPACEVALUELIST* ns, bool qualified, bool tagsOnly, bool allowUsing)
    {
        Optimizer::LIST* lst;
    ...
    static Optimizer::LIST* structuredArg(SYMBOL* sym, Optimizer::LIST* in, TYPE* tp) { return searchNS(sym, basetype(tp)->sp->sb->parentNameSpace, in); }
                ...
                else if (basetype(tpp)->type != basetype(tpa)->type)
                {
                    if (isint(tpa))
                        ...
                        else if (isint(tpp))
                        {
                            ...
                            // long is effectively the same as int on some Optimizer::architectures.   It prefers a mapping between the two to a
                                  ...
                                  bool maybeConversion, bool toInstantiate, int flags)
    {
        ...
        if (!sp || sp->sb->storage_class == sc_overloads)
        {
            ...
            SYMBOL *found1 = nullptr, *found2 = nullptr;
            if (!Optimizer::cparams.prm_cplusplus &&
                ((Optimizer::architecture != ARCHITECTURE_MSIL) || !Optimizer::cparams.msilAllowExtensions || (sp && !sp->tp->syms->table[0]->next)))
            {

...
if( de > max )
	...
	if( de > 1.0e-4f )
		...
		printf("x (%.9E,%.9E) z (%.9E,%.9E) y (%.9E,%.9E) max %.4E\n",
		       crealf(xsave), cimagf(xsave), crealf(z), cimagf(z),
		       crealf(y), cimagf(y), max);
		if( d->tstyp == POWER )
			{
			printf( "a (%.6E,%.6E)\n", crealf(a), cimagf(a) );
			}
 ...
 crealf(yy1), cimagf(yy1), crealf(y2), cimagf(y2),
 crealf(y3), cimagf(y3), crealf(y4), cimagf(y4), k, crealf(xsave), cimagf(xsave) );
			}
		}

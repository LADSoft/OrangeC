...
{
	return spawnbase(path,&argv0,0,P_OVERLAY,0);
}
int _RTL_FUNC execle(const char *path, const char *argv0,...)
{
	return spawnbase(path,&argv0,findenv(&argv0),P_OVERLAY,0);
}
int _RTL_FUNC execlp(const char *path, const char *argv0,...)
{
	return spawnbase(path,&argv0,0,P_OVERLAY,1);
}

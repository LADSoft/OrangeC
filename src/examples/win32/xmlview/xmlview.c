...
void PopulateAttribs(HWND hwndTree, HWND below, struct xmlAttr *attribs) 
{
    while (attribs) {
        ...
        t.u.item.mask = TVIF_TEXT  | TVIF_PARAM | TVIF_IMAGE | TVIF_SELECTEDIMAGE;
        t.u.item.hItem = 0;	
        t.u.item.pszText = attribs->userdata ;
        t.u.item.cchTextMax = strlen(attribs->userdata) ;
        t.u.item.lParam = attribs ;
        t.u.item.iImage = ilAttrib ;
        t.u.item.iSelectedImage = ilAttrib ;
...
void PopulateTree(HWND hwndTree, HWND below, struct xmlNode *nodes)
{
    while (nodes) {
        ...
        t.u.item.mask = TVIF_TEXT  | TVIF_PARAM | TVIF_IMAGE | TVIF_SELECTEDIMAGE;
        t.u.item.hItem = 0;	
        t.u.item.pszText = nodes->userdata ;
        t.u.item.cchTextMax = strlen(nodes->userdata) ;
        t.u.item.lParam = nodes ;
        t.u.item.iImage = ilNode ;
        t.u.item.iSelectedImage = ilNode ;

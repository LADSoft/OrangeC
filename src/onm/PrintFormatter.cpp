...
void PrintFormatter::Print(SymbolTable &symbols)
{
    ...
    if (filePerLine)
    {
        ...
        for (auto s : symbols.symbols)
        {
            std::cout << GetOffset(s, rad, maxOffs) << " " << GetType(s) << " " << std::setw(symbolSize + 1) << (demangle ? s->displayName : s->symbolName) << " " << s->fileName << std::endl;             
        }
    }
    else
    {
        ...
        for (auto s : symbols.symbols)
        {
            ...
            std::cout << GetOffset(s, rad, maxOffs) << " " << GetType(s) << " " << (demangle ? s->displayName : s->symbolName) << std::endl;             
        }
    }
}

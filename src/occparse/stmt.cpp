...
namespace Parser
{
    ...
    void InsertLineData(int lineno, int fileindex, const char* fname, char* line)
    {
        ...
        ld = (Optimizer::LINEDATA*)Alloc(sizeof(Optimizer::LINEDATA));
        ld->file = fname;
                                        ...
                                        bool* declaration)
    {
        ...
        if (startOfType(lex, false) && (!Optimizer::cparams.prm_cplusplus || resolveToDeclaration(lex)))
        {
            if (declaration)
                *declaration = true;
            if ((Optimizer::cparams.prm_cplusplus && kw != kw_do && kw != kw_else) || (Optimizer::cparams.prm_c99 && (kw == kw_for || kw == kw_rangefor)))
            {
                // empty
            }
            else
            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (Optimizer::cparams.prm_debug || Optimizer::cparams.prm_optimize_for_size || (Optimizer::chosenAssembler->arch->denyopts & DO_NOENTRYIF))
                        {
            ...
            if (isstructured(tp) || basetype(tp)->type == bt_memberptr)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if ((Optimizer::architecture != ARCHITECTURE_MSIL) || funcsp->sb->attribs.inheritable.linkage2 == lk_unmanaged ||
                            !msilManaged(funcsp))
                        {
    ...
    static LEXEME* statement_expr(LEXEME* lex, SYMBOL* funcsp, BLOCKDATA* parent)
    {
        ...
        EXPRESSION* select = nullptr;
        TYPE* tp = nullptr;
        Optimizer::LINEDATA *oldLineHead = linesHead, *oldLineTail = linesTail;
        linesHead = linesTail = nullptr;
        (void)parent;
    ...
    bool hasInlineAsm()
    {
        return Optimizer::architecture == ARCHITECTURE_X86;
    }
    LEXEME* statement_asm(LEXEME* lex, SYMBOL* funcsp, BLOCKDATA* parent)
    {
        ...
        if (hasInlineAsm())
        {
            ...
            else
            {
                ...
                if (lex)
                {
                    if (Optimizer::cparams.prm_assemble && (MATCHKW(lex, kw_public) || MATCHKW(lex, kw_extern) || MATCHKW(lex, kw_const)))
                    {
                        lex = asm_declare(lex);
                    }
                    else
        ...
        switch (KW(lex))
        {
            ...
            default:
                if (((startOfType(lex, false) &&
                      ((!Optimizer::cparams.prm_cplusplus && ((Optimizer::architecture != ARCHITECTURE_MSIL) || !Optimizer::cparams.msilAllowExtensions)) ||
                    ...
                    MATCHKW(lex, kw_namespace) || MATCHKW(lex, kw_using) || MATCHKW(lex, kw_decltype) || MATCHKW(lex, kw_static_assert))
                {
                    ...
                    while (((startOfType(lex, false) &&
                             ((!Optimizer::cparams.prm_cplusplus && (Optimizer::architecture != ARCHITECTURE_MSIL)) || resolveToDeclaration(lex)))) ||

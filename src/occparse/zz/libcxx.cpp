...
SYMBOL* MyMakeIntegerSeq(TEMPLATEPARAMLIST* args)
{
    if (isintconst(args->next->next->p->byNonType.val))
    {
        SYMBOL *tpl = args->p->byTemplate.val;
        if (tpl->sb->parentTemplate)
            tpl = tpl->sb->parentTemplate;
        int nt = basetype(args->next->p->byClass.val)->type + args->next->next->p->byNonType.val->v.i;
        const char *nm = args->p->byTemplate.val->name;
        auto sym = integerSequences[nm][nt];
        if (sym)
            return sym;
        int n = args->next->next->p->byNonType.val->v.i;
        decltype(args) args1;
        ...
        args1->next->next->p->byNonType.tp = args1->next->p->byClass.val;
        ...
        for (int i=0; i < n; i++, last = &(*last)->next)
        {
                *last = Allocate<TEMPLATEPARAMLIST>();
                ...
                (*last)->p->type = kw_int;
                (*last)->p->byNonType.tp = args1->next->p->byClass.val;
...
SYMBOL* MyTypePackElement(TEMPLATEPARAMLIST* args)
{
    if (isintconst(args->p->byNonType.val))
    {
        int n = args->p->byNonType.val->v.i;
        auto lst = args->next->p->byPack.pack;
        for ( ; n && lst ; n--)
        {
            lst = lst->next;
        }
        if (lst)
        {
            auto shim = Allocate<SYMBOL>();
            shim->name  = "";
            shim->tp = lst->p->byClass.val;
            return shim;
        }
    }
    return nullptr;

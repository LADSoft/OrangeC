...
class Plane : public DisplayObject
{
  public:
    Plane(int index, int Src, int Dest, int alt, bool fast, int start, int Fuel, int x = 0, int y = 0, Heading head) :
        ...
        circling(false),
        clearedToLand(false),
        nextMove(startTime + speed),
        heading(head)
    {
    }
    void CommandPlane(Command command, int modifier);
    void Rotate(int n) { rotate = Rotate; }
    void Circle(int x, int y);
    void SetHeading(enum Heading n)
    {
        heading = n;
        Circle(-1, -1);
    }
...
{
  public:
    Fix(int index, int x = 0, int y = 0, Heading head = east) : DisplayObject(index + '0', x, y), heading(head) {}
    bool At(Plane& plane);
...
{
  public:
    Airport(int index, int x = 0, int y = 0, Heading head = east) : DisplayObject(index ? '$' : '%', x, y), heading(head) {}

...
#define Button_GetState(hwndCtl) \
    ((int)(DWORD)SendMessage((hwndCtl), BM_GETSTATE, 0L, 0L))
...
#define Button_SetState(hwndCtl, state) \
    ((UINT)(DWORD)SendMessage((hwndCtl), BM_SETSTATE, (WPARAM)(int)(state), 0L))
...
#define ComboBox_AddItemData(hwndCtl, data) \
    ((int)(DWORD)SendMessage((hwndCtl), CB_ADDSTRING, 0L, (LPARAM)(data)))
...
#define ComboBox_LimitText(hwndCtl, cchLimit) \
    ((int)(DWORD)SendMessage((hwndCtl), CB_LIMITTEXT, (WPARAM)(int)(cchLimit), 0L))
...
#define Edit_FmtLines(hwndCtl, fAddEOL) \
    ((BOOL)(DWORD)SendMessage((hwndCtl), EM_FMTLINES, (WPARAM)(BOOL)(fAddEOL), 0L))
...
#define Edit_GetPasswordChar(hwndCtl) \
    ((TCHAR)(DWORD)SendMessage((hwndCtl), EM_GETPASSWORDCHAR, 0L, 0L))
...
#define GetFirstSibling(hwnd) \
    GetWindow(hwnd, GW_HWNDFIRST)

#define GetInstanceModule(hInstance) \
    (HMODULE)(hInstance)

#define GetLastSibling(hwnd) \
    GetWindow(hwnd, GW_HWNDLAST)

#define GetNextSibling(hwnd) \
    GetWindow(hwnd, GW_HWNDNEXT)

#define GetPrevSibling(hwnd) \
...
#define HANDLE_WM_MENUSELECT(hwnd, wParam, lParam, fn) \
    ((fn)((hwnd), (HMENU)(lParam), (int)(LOWORD(wParam)), (HIWORD(wParam) & MF_POPUP) ? GetSubMenu((HMENU)lParam, LOWORD(wParam)) : 0L, (UINT)(((short)HIWORD(wParam) == -1) ? 0xFFFFFFFF : HIWORD(wParam))), 0L)

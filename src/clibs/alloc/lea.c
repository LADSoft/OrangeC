/*
  ...
  comments, complaints, performance data, etc to dl@cs.oswego.edu_R
...
void* __PAD2(void* p, int size, int align)
{
     if (p)
     {
          int offset = (int)_HEAP_CHECK;
...
void *malloc(size_t bytes)
{
    char *rv;
    __ll_enter_critical();
    int len = __size(bytes);
    rv = __PAD(dlmalloc(len), bytes);
    __ll_exit_critical();
    return rv;
}
void *calloc(size_t n, size_t size)
{
    char *rv;
    __ll_enter_critical();
    int len = __size(n * size);
    rv = __PAD(dlcalloc(1, len), n * size);
    __ll_exit_critical();
    return rv;
}
void *realloc(void *old, size_t bytes)
{
    ...
    __ll_enter_critical();
    int len = __size(bytes);
    rv = __PAD(dlrealloc(__CHECK(rv), len), bytes);
    __ll_exit_critical();
    return rv;
}
void free(void *mem)
...
void* _RTL_FUNC aligned_alloc(size_t alignment, size_t size) 
{
    __ll_enter_critical();
    int len = __size(size+alignment);
    void * rv = __PAD2(dlmemalign(alignment, len), size, alignment);
    __ll_exit_critical(); 
    return rv; 
}
void* _RTL_FUNC _mm_malloc(size_t size, size_t alignment) 
{ 
    __ll_enter_critical();
    int len = __size(size + alignment);
    void * rv = __PAD2(dlmemalign(alignment, len), size, alignment);
...
void* _RTL_FUNC _aligned_malloc(size_t alignment, size_t size) 
{
    __ll_enter_critical();
    int len = __size(size + alignment);
    void * rv = __PAD2(dlmemalign(alignment, len), size, alignment);
...
void _RTL_FUNC _aligned_free(void* ptr) 
{ 
    __ll_enter_critical();
    dlfree(__CHECK(ptr));
    __ll_exit_critical(); 
}
void _RTL_FUNC _mm_free(void* ptr) 
{ 
    __ll_enter_critical();
    dlfree(__CHECK(ptr));
    __ll_exit_critical(); 
}
void _RTL_FUNC cfree(void* ptr) 
{ 
    __ll_enter_critical();
    dlfree(__CHECK(ptr));
    __ll_exit_critical(); 
...
static struct PoolList *pools = NULL;
...
void *ccdlMoreCore(int size)
{
    ...
    if (size > 0)
    {
        struct PoolList *pool; 
        size += sizeof(struct PoolList) + RM_PAGE_MASK;
          if (size < MINIMUM_MORECORE_SIZE)
             size = MINIMUM_MORECORE_SIZE;
        ptr = __ll_malloc(size);
        if (ptr == 0)
          {
            return (void *) MFAIL;
        }
        // save ptrs so they can be freed during cleanup
        pool = (struct PoolList *) ptr;
        pool->next = pools;
        pools = pool;

        ptr = (void *) ((((size_t) ptr) + sizeof(struct PoolList ) + RM_PAGE_MASK) & -RM_PAGE_MASK);
        sbrk_top = (char *) pool + size;
        return ptr;
    }
    else if (size < 0)
    {

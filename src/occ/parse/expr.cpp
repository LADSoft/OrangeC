 ...
 *     the Free Softwoccare Foundation, either version 3 of the License, or
...
static LEXEME* variableName(LEXEME* lex, SYMBOL* funcsp, TYPE* atp, TYPE** tp, EXPRESSION** exp, BOOLEAN* ismutable, int flags)
{
    ...
    else
    {
        ...
        if (MATCHKW(lex, openpa))
        {
            ...
            if (sp->storage_class != sc_overloads)
            {
                InsertSymbol(sp, sp->storage_class, lk_none, FALSE);
...
LEXEME* expression_unary(LEXEME* lex, SYMBOL* funcsp, TYPE* atp, TYPE** tp, EXPRESSION** exp, BOOLEAN* ismutable, int flags)
{
    ...
    switch (kw)
    {
        ...
        case notx:
            ...
            {
                if ((cparams.prm_cplusplus || chosenAssembler->msil) &&
                    insertOperatorFunc(ovcl_unary_numericptr, notx, funcsp, tp, exp, NULL, NULL, NULL, flags))
                {
                }
                else
                {
        ...
        case complx:
            ...
            {
                if ((cparams.prm_cplusplus || chosenAssembler->msil) &&
                    insertOperatorFunc(ovcl_unary_int, complx, funcsp, tp, exp, NULL, NULL, NULL, flags))
                {
                }
                else
                {

...
int _RTL_FUNC _wstat32i64(wchar_t* path, struct _stat32i64* __statbuf)
{
    int fd, rv, rootdir;
    char pbuf[265];
...
int _RTL_FUNC _stat32i64(char* path, struct _stat32i64* __statbuf) 
{
    wchar_t buf[256], *q = buf;
    while (*path)
        *q++ = *path++;
    *q = *path;
    return _wstat32i64(buf, __statbuf);
}

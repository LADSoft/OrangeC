...
class ppDefine
{
  public:
    ...
    class Definition : public Symbol
    {
      public:
        ...
        Definition& operator=(const Definition& old)
        {
  ...
  public:
    ...
    Definition* Lookup(const std::string& name) { return static_cast<Definition*>(symtab.Lookup(name)); }
    int Process(std::string& line, bool leavePlaceholder = false)
    {
        ...
        for (p = 0; p < line.size(); p++)
        {
            ...
            if (line[p] == REPLACED_TOKENIZING || (!leavePlaceholder && line[p] == MACRO_PLACEHOLDER) ||
    ...
    std::string defid(const std::string& macroname, int& i, int& j)
    /*
     ...
     */
    {
        ...
        while (j < macroname.size() && isSymbolChar(macroname.c_str() + j, false))
        {
    ...
    void SyntaxError(const std::string& name) { Errors::Error(std::string("Wrong number of arguments in call to macro ") + name); }
...
template <typename T, bool(isSymbolChar)(const char*, bool)>
void ppDefine<T, isSymbolChar>::Stringize(std::string& macro)
{
    std::string repl;
    int waiting = 0;
    int last = 0, pos;
...
template <typename T, bool(isSymbolChar)(const char*, bool)>
bool ppDefine<T, isSymbolChar>::Tokenize(std::string& macro)
{
    bool rv = false;
    int waiting = 0;
...
template <typename T, bool(isSymbolChar)(const char*, bool)>
int ppDefine<T, isSymbolChar>::InsertReplacementString(std::string& macro, int end, int begin, std::string text, std::string etext)
{
    int q;
...
template <typename T, bool(isSymbolChar)(const char*, bool)>
bool ppDefine<T, isSymbolChar>::NotSlashed(const std::string& macro, int pos)
{
    int count = 0;
    while (pos && macro[--pos] == '\\')
        count++;
...
template <typename T, bool(isSymbolChar)(const char*, bool)>
bool ppDefine<T, isSymbolChar>::ppNumber(const std::string& macro, int start, int pos)
{
    int x = pos;
    if (pos <= 0)
        return false;
...
/* replace macro args */
template <typename T, bool(isSymbolChar)(const char*, bool)>
bool ppDefine<T, isSymbolChar>::ReplaceArgs(std::string& macro, const DefinitionArgList& oldargs, const DefinitionArgList& newargs,
...
template <typename T, bool(isSymbolChar)(const char*, bool)>
void ppDefine<T, isSymbolChar>::SetupAlreadyReplaced(std::string& macro)
{
    bool instr = false;
    for (int p = 0; p < macro.size(); p++)
    {
...
template <typename T, bool(isSymbolChar)(const char*, bool)>
int ppDefine<T, isSymbolChar>::ReplaceSegment(std::string& line, int begin, int end, int& pptr, bool eol,
                                              std::deque<Definition*>& definitions, std::deque<TokenPos>* positions)
{
    std::string name;
    int waiting = 0;
...
void ppDefine<T, isSymbolChar>::ParseAsmSubstitutions(std::string& line)
{
    int quote = 0;
...
void ppDefine<T, isSymbolChar>::ReplaceAsmMacros(std::string& line)
{

    int n = line.find_first_not_of(" \t\r\v\n");
    while (n != std::string::npos && isSymbolChar(line.c_str() + n, true))
    {
        int n1 = n;
        while (n1 != line.size() && isSymbolChar(line.c_str() + n1, false))
        {
...
template <typename T, bool(isSymbolChar)(const char*, bool)>
void ppDefine<T, isSymbolChar>::replaceDefined(std::string& line)
{
    ...
    while (n != std::string::npos)
    {
        ...
        while (m < line.size() && isspace(line[m]))
            m++;
        if (isSymbolChar(line.c_str() + m, true))
        {
            int q = m++;
            while (isSymbolChar(line.c_str() + m, false))
                m++;
